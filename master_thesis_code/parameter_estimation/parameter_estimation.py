...
class ParameterEstimation:
    ...
    def __init__(
        self,
        wave_generation_type: WaveGeneratorType,
        use_gpu: bool,
    ...
    def generate_waveform(
        self, use_antenna_pattern_functions: bool = True
    ) -> cp.ndarray:
        waveform = self.waveform_generator(
            **self.parameter_space._parameters_to_dict(), dt=self.dt, T=self.T
        )

        if use_antenna_pattern_functions:
            return_waveform = self.lisa_configuration.transform_from_ssb_to_lisa_frame(
                waveform=waveform
            )
        else:
    ...
    def _crop_to_same_length(signals: typing.List[cp.array]) -> typing.List[cp.array]:
        minimal_length = min(
            [
                min(len(measurements[0]), len(measurements[1]))
                for measurements in signals
            ]
        )
        signals = cp.array(
            [
                [measurements[0][:minimal_length], measurements[1][:minimal_length]]
                for measurements in signals
            ]
        )
        return signals
    ...
    def five_point_stencil_derivative(self, parameter_symbol: str) -> cp.array:
        ...
        waveform_derivative = (
            (-waveforms[3] + 8 * waveforms[2] - 8 * waveforms[1] + waveforms[0])
            / 12
            / derivative_epsilon
        )
    ...
    def _plot_waveform(
        ...
        if parameter_symbol is not None:
            parameter_value = str(
                np.round(getattr(self.parameter_space, parameter_symbol), 3)
            )
    ...
    def compute_fisher_information_matrix(self) -> cp.ndarray:
        ...
        for parameter_symbol in parameter_symbol_list:
            waveform_derivative = self.five_point_stencil_derivative(
    ...
    def check_parameter_dependency(self, parameter_symbol: str, steps: int = 5):
        ...
        parameter_configuration = next(
            (
                config
                for config in self.parameter_space.parameters_configuration
                if config.symbol == parameter_symbol
            ),
            None,
        )

        self.parameter_space.a = 0.0
        self.parameter_space.x0 = 1.0
        self.parameter_space.Phi_theta0 = 0.0

        if parameter_configuration is None:
            raise ParameterEstimationError(
                "check_parameter_dependency couldn't match parameter symbol."
            )

        parameter_steps = cp.linspace(
            parameter_configuration.lower_limit,
            parameter_configuration.upper_limit,
            steps,
        )
        ...
        for i, parameter_step in enumerate(parameter_steps):
            ...
            waveforms.append(
                self.generate_waveform(use_antenna_pattern_functions=False)
            )
            _LOGGER.info(
                f"Parameter dependency for {parameter_symbol}: {i+1}/{len(parameter_steps)} waveforms generated."
            )

        self._plot_waveform(
            waveforms=waveforms,
            plot_name=f"dependency_{parameter_symbol}",
            use_log_scale=True,
        )

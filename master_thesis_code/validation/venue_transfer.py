...
:mod:`master_thesis_code.validation.calibration_gate` (code identity
``065e7f58``, run ``dbde71dc``) and
:mod:`master_thesis_code.validation.closed_loop_gfrac` (``77b524af``) are
...
2. *Chunked pair evaluation* (registered implementation freedom, prereg §3):
   ...
   :func:`~master_thesis_code.bayesian_inference.bayesian_statistics.completion_mass_factor_g`
...
class VenueContext:
    ...
    Wraps the gate's :class:`~master_thesis_code.validation.calibration_gate.GateContext`
...
def draw_ball_pinned(
    ...
    The gate's :func:`~master_thesis_code.validation.calibration_gate.draw_ball`
    ...
    Returns:
        The :class:`~master_thesis_code.validation.calibration_gate.HostBall`
...
def log_channel_posteriors_ball_sigma_vector(
    ...
    *,
    ...
    The gate's
    :func:`~master_thesis_code.validation.calibration_gate.log_channel_posteriors_ball`
...
def run_validate(*, n_events_cap: int = 40, skip_v_t5: bool = False) -> dict[str, Any]:
    ...
    out["v_t4"] = {
        "note": (
            ...
            "unit-tested in master_thesis_code_test/validation/test_venue_transfer.py"
        )
    }
    return out

...
class LISAConfiguration:
    ...
    def F_plus_del_theta(self, t: float) -> float:
        ...
        """
        return (
            cp.sin(2*self.phi_solar_barycenter(t))*cp.sin(2*self.psi_solar_barycenter(t))*cp.sin(self.theta_solar_barycenter(t)) 
            - cp.sin(self.theta_solar_barycenter(t))*cp.cos(2*self.phi_solar_barycenter(t))*cp.cos(2*self.psi_solar_barycenter(t))*self.cos_theta_solar_barycenter(t))
    ...
    def F_plus_del_phi(self, t: float) -> float:
        ...
        """
        return (
            -2*(self.cos_theta_solar_barycenter(t)**2/2 + 1/2)*cp.sin(2*self.phi_solar_barycenter(t))*cp.cos(2*self.psi_solar_barycenter(t)) 
            - 2*cp.sin(2*self.psi_solar_barycenter(t))*cp.cos(2*self.phi_solar_barycenter(t))*self.cos_theta_solar_barycenter(t))
    ...
    def phi_t(self, t: float) -> float:
        ...
        return cp.multiply(t, 2*np.pi/T)
    ...
    def power_spectral_density_instrumental(self, f: float) -> float:
        ...
        return A_1*(self.P_OMS(f) + 2.*(1. + cp.cos(f/f_ast)**2) * self.P_acc(f) / (2.*np.pi*f)**4) * (1. + 6./10. * (f/f_ast)**2)

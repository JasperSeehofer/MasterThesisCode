...
| **2026-05-05 (pipeline change — Re-evaluate)** | `[PHYSICS]` 1D + 2D detection probabilities → principled monotonic-asymptotic extrapolation | (uncommitted at writing) | n/a | n/a | n/a | n/a | Replaces Phase 45 Plan 45-02/04 anchor scheme (1D Wilson 95% LB + intermediate empirical anchor) with a principled bridge from (dl_min, p_edge) to (0, 1) and slope-matched + clamped suppressing-face extrapolation. **All `posteriors{,_with_bh_mass}/` produced before this commit are stale** for any conclusion that depends on absolute p_det values near the d_L grid edges. See `.planning/2D-CHANNEL-AUDIT-20260505.md` for the audit + rationale. |
| *(pending)* | post-fix h=0.73 closure validation | next | 21-pt fine | 1549 | — | — | cpu_il, queued after code push; isolates the principled-extrapolation fix's effect at the loudest pre-fix outlier (h=0.73 2D z=+37 → expected drop to ≤2σ) |

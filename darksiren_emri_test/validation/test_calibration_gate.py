...
from darksiren_emri.validation import pp_coverage as pp
...
def test_classify_porcelain_splits_import_path_from_other_dirt() -> None:
    ...
    status = (
        " M darksiren_emri/validation/calibration_gate.py\n"
        "?? darksiren_emri_test/validation/new_test.py\n"
        "R  docs/a.md -> darksiren_emri/moved.py\n"
        " M CLAUDE.md\n"
    ...
    assert all("darksiren_emri" in line for line in dirt["import_path"])
...
def test_enforce_clean_import_path_refuses_and_inventories(
    ...
    monkeypatch.setattr(
        cg,
        "_git_state",
        lambda: ("abc123", {"import_path": [" M darksiren_emri/a.py"], "other": []}),
    )
...
def test_run_cell_refuses_dirty_import_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        cg,
        "_git_state",
        lambda: ("abc123", {"import_path": ["?? darksiren_emri/new.py"], "other": []}),
    )
    gcfg = cg.GateConfig(cell="B2", h_true=0.730, ball=True, lambda_ball=4.0, sigma_z=0.035)
    with pytest.raises(SystemExit, match="import path"):

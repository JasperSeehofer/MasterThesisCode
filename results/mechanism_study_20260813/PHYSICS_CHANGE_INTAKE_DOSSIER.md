...
## Erratum E1 (2026-08-14) — constraint repairs and register extension (ledger row #102; prospective, pre-use)

Applied before the dossier is first used as a mechanical filter; no completed measurement is re-scored. **(i) C1 repaired:** "EXACT … not a small residual" over-claims grid resolution — refined f_host = 0 residuals are ~1e-4, so the data **bound** the host-gate term (|residual| ≲ 1e-4 at zero host dose), they do not annihilate it. Candidates predicting a sub-1e-4 residual at f_h = 0 are C1-consistent. **(ii) C2/C2′ repaired:** as written the pair is jointly unsatisfiable under the any-row-fails-refutes rule (strict linearity and more-than-first-order cannot both hold), and the diagonal R_dose (1.214 / 0.766 / 0.949, non-constant at 6.1σ) already kills strict linearity as a functional-form law. Replacement: C2 = leading-order behaviour consistent with the measured R_dose band at S33 ([0.75, 1.25]); C2′ = no requirement of strict linearity; functional-form claims finer than the registered resolution floor remain barred. **(iii) Register extension:** two candidate classes surfaced by the commission from committed data are added pending their own pre-registration: **M6** — σ_z-blind aggregate log-posterior tilt × dose-controlled curvature composite (gradient at truth dose-invariant at 2625–2720 nats/h; predicted α-share 52.7% vs 53.3% measured; survives the three pre-specified kill tests); **M7** — host/impostor ball-window inclusion asymmetry (named in this dossier's own parity text, previously unassigned). Consequence: "M2′ is the register's only surviving candidate" is retired; the open set is {M2′, M6, M7}.

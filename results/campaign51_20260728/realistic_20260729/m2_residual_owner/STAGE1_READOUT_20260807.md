...
## LOOPHOLE READS A1/A2 (2026-08-08)

Both free reads from §5(A) were executed and independently adjudicated (`a1_graph_signflip.py`/`a1_results.json`, `a2_completion_functional.py`/`a2_results.json`, adversarial re-implementation `adjudicate_a1_a2.py`/`adjudicate_a1_a2_results.json`, fresh seeds/N_PERM, git HEAD `e253e0c1`). Adjudication rule from §0 applies: **where the verifier filed a discrepancy, verifier numbers/readings win.**

### A1 — H-e decisive test (component-level sign-flip)

- **A1's own reading (win279 graph — the 279 sky+2σ-d_L pairs that literally define C-4 "overlap", 139 components over the 385 overlap events, largest 10):** PASS at both venues — iiib p ≈ 0.0030–0.0035, joint_r1 p ≈ 0.0020–0.0029 across independent seeds/implementations, both ≪ α = 0.0455. Jackknife-over-components: no sign flip on any single-component removal at either venue; top-component share of the signed sum ≤ 17.3%. **A1 verdict: CONFIRMED.**
- **[VERIFIER] material addition, not disclosed in A1's caveats:** the intake's own H-e prose names the linking structure as "the 1620-sky-pair graph," distinct from the Refute-by's literal "C-4 overlap graph" (279 pairs) that A1 tested. Under the full 1620-sky-pair graph with pass-through linkage via non-overlap events (53 components, one giant component = 176/385 = 45.7% of overlap events), the component sign-flip test **FAILS at both venues**: iiib p ≈ 0.069, joint_r1 p ≈ 0.062, both ≥ α. Diagnosed as a power collapse of a 53-unit test (giant-component share of the signed sum, 0.56–0.57, is proportionate to its 0.457 event share — not enriched; no jackknife sign flip; max LOCO 19–20%), i.e. the H-e-predicted localization signature ("mean driven by ≤ a few components") is **absent even under this reading**. Two further graph readings (sky-restricted-to-overlap-endpoints; merged win-components+shared-control) both PASS decisively. **Net: 3 of 4 defensible exchangeability models close H-e; the 4th (maximal pass-through linkage) does not, on power grounds, not on the mechanism's predicted signature.**
- Two minor discrepancies filed and immaterial to the verdict: (i) A1's "single largest component" label for the top-LOCO component (id 1412, size 9) is imprecise — the true largest win279 component is size 10, a different id; the reported numbers are correct. (ii) A1's RNG-seed reading of "re-matching under different RNG seeds" (rather than re-deriving the deterministic 1-NN match) is confirmed harmless — matching is provably tie-free and deterministic, and the harder literal reading (stochastic k=3-NN re-matching) still passes at both venues in all 6 seed cells.
- **A1 branch outcome (verified): H-E-STAYS-CLOSED, but conditionally — closure holds on the literal Refute-by object (A1's win279 graph) and on 2 of 3 alternative exchangeability readings tested by the verifier; it does not hold unconditionally against the intake's own broader "1620-sky-pair graph" prose under maximal pass-through linkage, where the test loses power rather than exhibiting the H-e signature.**

### A2 — bundle-functional sanity read

- Chord (T_Lcomp carrier, both pure and g_frac-composed variants) is **smooth, strictly monotone, but not deterministic** in (log10 d_L, log10 rel-err): Spearman vs log10 d_L ≈ +0.96; decile profile strictly monotone, zero sign changes; primary deg-3 polynomial R² = 0.883/0.875 (c_pure/c_gfrac) — **below the read's own pre-stated 0.95 smoothness bar**, confirmed robust to degree (saturates by deg 3–4), binning (8×8/12×12/16×16/20×20), and 5-fold CV (near-zero overfitting, CV R² 0.870–0.881). Residual scatter (RMS 0.0546 nats) is 10⁴–10¹³× the CSV precision floor — real structure, not quantization.
- Composition-chain reproduction of the matched residual: **~65–67%** (ratio 0.653–0.666 at the two venues) — **outside the pre-stated 0.70–1.30 band at both venues** (gaps ≈ −0.94σ/−1.01σ cluster-robust). Carrier-level fit-residual (fixed d_L, rel-err) matched diff = +0.0083 ± 0.0029 (2.9σ) — a genuine, stratum-specific, non-d_L/σ_dL component of the residual, corroborated by two adjudication attacks: a placebo control-vs-control re-match shows no spurious residual (−0.0051 ± 0.0050, consistent with zero), and D-1's independent d_L-matched sensitivity re-match yields the same ~0.006–0.007 nats orthogonal-to-(d_L,σ_dL) leftover via a fully separate route.
- **upgrade_met = false (verified).** Per the pre-stated §5(A)(2) criterion, "confounding-absorbable" does **not** upgrade to "explained: d_L-dependent completion-leg h-response × stratum d_L composition." A2 does, however, positively strengthen the H-c modified account: ~2/3 of the matched residual is accounted for by a smooth, strongly d_L-dominated (not purely d_L) completion-leg functional response composed with the stratum's d_L imbalance; the remaining ~1/3 (≈ 0.006–0.008 nats, ~2–3 cluster-σ) is real chord structure orthogonal to (d_L, σ_dL) and is left for the wider collinear density/latitude bundle (§4.1) that D-2's m2/m3 rungs and the verifier's m_geo rung already show can absorb it — i.e. A2 narrows, but does not eliminate, the undecided family-attribution question of §4.1.
- **A2 verdict: CONFIRMED** (all headline numbers reproduced exactly; verified `upgrade_met=false`; no numeric discrepancies filed).

### Resulting stage-1 verdict (final, awaiting author stage-5 ruling)

Both loophole reads close in the direction anticipated by the stage-1 finding, with one qualification each surfaced only by adjudication:

1. **H-e (chance/multiplicity):** DISFAVORED → now **CONDITIONALLY CLOSED**. The pre-declared decisive test was run and passes on its literal object and on the majority of defensible exchangeability readings (3/4); it does not pass under the intake's own alternative "1620-sky-pair" framing of the linking structure, where the failure mode is loss of power in a 53-unit test rather than the H-e-predicted coherent-localization-absence signature. The chronicle may state H-e is refuted, but should disclose this as a majority-of-readings result, not an unconditional one.
2. **H-c (modified — confounding-absorbable, collinear d_L-geometry + ball-density bundle):** remains the surviving account and is now **functionally grounded rather than purely a matching-collapse observation** — the completion-leg h-response is demonstrably a smooth, near-monotone (but not deterministic) function of d_L that mechanically explains ~2/3 of the matched residual through stratum d_L-composition alone. It does **not** reach the upgraded "fully explained" status the readout named as the alternative outcome; ~1/3 of the residual is real chord structure not captured by (d_L, σ_dL), consistent with (and not distinguishable from) the wider density/latitude bundle already implicated by D-2/verifier m_geo.
3. **No mechanism owner is reinstated.** Neither loophole read reopens H-a, H-b, or H-d as owners, nor implicates the NEGLECT-registered Eq. (31) cross-term; nothing here changes §3's exclusion list.

**Formulation offered to the author for the stage-5 ruling (not itself a ruling):** the thread is ready to close as a **confounding/dissolution chronicle verdict in modified-H-c form, conditionally qualified on H-e's graph-reading dependence and on the ~1/3 unexplained-by-(d_L,σ_dL) chord residue** — i.e., "the residual is real and component-coherent, is not a chance artifact of correlated pairs under the literal and majority-reading tests, and is confounding-absorbable by a d_L-geometry/ball-density bundle whose completion-leg mechanism is now shown to be ~two-thirds a smooth d_L-functional effect and ~one-third unresolved structure within that same bundle family" — pending the author's decision on whether the two adjudication-surfaced qualifications (A1's graph-reading conditionality; A2's sub-band full-chain ratio) require their own disclosure line in the book text, and on whether instrument (B) of §5 is still wanted to resolve the remaining ~1/3 / family-attribution question before chronicling.

**Provenance:** `a1_results.json`, `a2_results.json`, `adjudicate_a1_a2_results.json` (this directory); git HEAD at read time `e253e0c1`. Free-read rails honored throughout (existing CSVs/JSONs only; no likelihood evaluations; no cluster jobs; no edits to `CLAIM_M2_RESIDUAL_OWNER_20260807.md`, the PHYSICS-GATE-LEDGER, or the book).

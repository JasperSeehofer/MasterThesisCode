...
## Row #320 — 2026-09-02 — **m-s0b-production launch STOPPED (third disciplined stop of the batch): the registered PA-HIER-31(d) node set is INEXPRESSIBLE in the driver as built (±0.033 re-derived b-nodes vs hardcoded ±0.02, registered never-interchangeable), AND the cost anchors conflict (orchestrator-derived ≤20 CPU-h vs the prereg's own §7.2 anchor 14.93-22.9 CPU-h/node → 74.7-101.4 CPU-h across the registered 5 nodes). Driver build dispatched; the cost word returns to the author as docket item 10.** Source: `graph1_20260901/exec/m-s0b-production/LAUNCH_RECORD.md`. **Preconditions confirmed met** — "Both preconditions read as satisfied for launch. This record does not dispute that; the STOP below is a separate, newly-found gap": g-byte-id discharged (row #319), g-score-null evaluable at run time. **The registered 5-node shape, quoted (PA-HIER-31(d)):** "Four θ-nodes at h = 0.730 on venue **iiib** (production, CoR-P), plus the truth node (θ=(0,1)) which doubles as C0: `truth (0,1)=C0`, `b_plus_re (+0.033,1)`, `b_minus_re (−0.033,1)`, `s_plus (0,√2)`, `s_minus (0,1/√2)`" — CoR-P-faithful, `theta_sites="2.2"`, `smear_global_selection=False`. **The never-interchangeable rule, quoted (PA-HIER-31 §2.1(a)):** "The two arms (±0.02 as-built vs ±0.033 re-derived) are never combined into one secant, one Z, or one materiality read; each is reported against its own registered grid." **The gap:** `hier_s0_driver.py`'s `THETA_NODES` dict is hardcoded to ±0.02 b-nodes only, "no `b_plus_re`/`b_minus_re` entry, and grepping the whole file for `0.033`, `0.0661`, `b_half_width`... or any CLI flag... returns **nothing**"; the only precedent, `--s-half-step`, "does not touch the b-axis at all." "**The b-node re-derivation from `PA-HIER-31(a)`/(d) is not mentioned anywhere in [`b-pahier33-scorer/RECORD.md`] — built, checked, or deferred. It is a silent gap, not a disclosed one.**" **The silent-wrong-secant risk the launcher refused:** running today would "score the as-built ±0.02 b-secant, not the registered ±0.033 b-secant — a different statistic," since the B0-B power band `σ_b < 0.0661` and materiality threshold `|b̂| < 0.0165` "are both stated in terms of the 0.033 step and do not transfer to a 0.02 step without re-derivation" — "exactly the class of substitution the task brief instructs against." Row #287/the byte-id discharge do not cover this: they certify "the b0i/S0-A default path... at current HEAD — they say nothing about whether the ±0.033 nodes were ever added, because they were never asked to be." **Cost-anchor conflict, both sources quoted:** the mirror-venue precursor anchor gives "`5 × 0.2843 ≈ 1.42` CPU-h at the mirror-venue per-cell rate" but "PA-HIER-31(d)'s own registered range for the *production* (iiib) venue is materially higher (§7.2's own anchor: 14.93-22.9 CPU-h **per theta-node** on the full production set, i.e. 74.7-101.4 CPU-h across 5 nodes... That upper figure alone exceeds the ≤20 CPU-h ORCHESTRATOR-DERIVED cap... by 3.7×-5×." "Which anchor governs is itself a fresh question for the author, not decided here." **Launcher's three author-facing options (none chosen):** (1) [DO] a small driver amendment adding `b_plus_re`/`b_minus_re` at ±0.033 as opt-in nodes; (2) [RULE] rule the as-built ±0.02 grid acceptable for S0-B (a genuine scope change to PA-HIER-31(a)'s "never combined" language, needing its own PA-HIER amendment); (3) descope S0-B's first pass to the s-axis + truth/C0 only (3 nodes, already byte-identically supported), treating the b-axis re-derivation as a follow-on build item. **STOP: no sbatch submitted, no job ID, no out-root created.** **Chair disposition (chair-derived):** the ±0.033 expressibility gap is a mechanical build of a REGISTERED value — dispatched as a driver build under row #290 row 6's driver-build scope, no fresh author word needed for that half. The COST word is not the chair's to derive past — it returns to the author as **docket item 10** (chair recommendation: adopt the prereg's own sourced anchor, cap ~105 CPU-h).

## Row #321 — 2026-09-02 — **m-s3 cell T invocation 1 CRASHED at 5/25 universes (INSTRUMENT-DEFECT path per the frozen stop rule: no sidecar written); classified a transient resource event (OOM-kill signature); RESUMED as invocation 2 of ≤3 per the frozen rule's own resume-to-complete provision.** Source: `graph1_20260901/exec/m-s3-postflip-coverage/cellT_inv1.log` (chair-read; the classification below is chair-derived, not instrument output). **Crash signature, quoted/confirmed directly from the log:** repeated `BrokenPipeError: [Errno 32] Broken pipe` inside `multiprocessing/connection.py:_send`, raised from multiple `ForkServerPoolWorker` processes (worker-27 among them) both on the initial `put((job, i, result))` and again "During handling of the above exception" on the wrapped-failure `put`; the shutdown warning "**resource_tracker: There appear to be 6 leaked semaphore objects to clean up at shutdown**: `{'/loky-2428303-8blz8xuv', '/loky-2428303-hewtc19d', '/loky-2428303-ve61_9o9', '/loky-2428303-e7xtvgct', '/loky-2428303-u1qhi0p2', '/loky-2428303-qvag_4w_'}`" — 6 workers lost simultaneously. **0 `b8_cal_harness` frames in the traceback beyond the 5 benign per-universe completion markers** (seeds 902000-902004, `done in ...s ->` lines, `n_scored no_bh=178-188` each), and **no `_run_status_T.json` sidecar was written** (confirmed absent on disk) — "the classic silent-parent OOM-kill signature," per the chair's classification. **Context:** three heavy local processes were concurrent at crash time — cell S, cell T, and the row #319 same-machine discharge run — on a 30G machine; the discharge run has since ended. **Known-benign log spam noted, not the cause:** 459 occurrences of the pre-existing ">5% quadrature-weight-outside-P_det-grid" warning family (numerator=0.000), unrelated to the crash. **Frozen registration §3 disposition applied mechanically (row #297/#301):** sidecar absent → INSTRUMENT-DEFECT; instrument-defect events do not consume a revision; resume-to-complete within ≤3 invocations × 86400 s each. **Invocation 2 launched by the chair** (orchestrator-as-runner; same flags/work-root `tree2_20260830/b8_cal_harness_work_s4_postflip/`, checkpoints skip automatically, 5 banked kept — confirmed live, PID 2794349, started 15:05, running); log `cellT_inv2.log`; chair monitor armed. **Cell S unaffected** — confirmed alive throughout (PID 2428302), 7 checkpoints banked at time of writing. **Falsification condition, stated plainly:** if invocation 2 crashes again at the same point (5/25), the transient classification is falsified and the defect returns as a fresh instrument question, not a second silent resume.

## Row #322 — 2026-09-02 — **falsifier (ii) ADJUDICATING READ COMPLETE (wave-2 decisive-verifier, top-tier): mechanical outcome INSIDE-BOTH — the falsifier does NOT refute c-a4-structural; d-a4-final-ratification returns to the author with numbers as docket item 11.** Source: `graph1_20260901/exec/v-falsifier-ii-classG/ADJUDICATING_READ_RECORD.md`. "This document is EVIDENCE for the author's `d-a4-final-ratification` [RULE] — it ratifies nothing and promotes no claim." **Fleet verification:** job 6769177 (array 0-32) **33/33 `COMPLETED 0:0`**; out-root `66/66 <arm>_<seed>_meta.json` present; commit stamp uniform `git_commit = c83e391d...` (66/66), `tree_dirty_incl_instrument = "clean"` (66/66), `git merge-base --is-ancestor 2b657255 c83e391d → true` — "the fleet ran at a commit containing the landed Option A′ repair"; arm flags uniform per arm (`mz_sel` all bt, `off` all bc, ~16% realized LHS2 difference between arms); catalogue pin `catalogue_pin_ok: true` 66/66; retrieval bundle **134/134 md5 OK**, explicit file list per the row #311 gotcha (no symlink leakage), fresh out-root confirmed (no PA-CA-11 reuse). **Registered reads, independence discipline applied — stage lhs2d run verbatim AND every number re-derived independently from md5-verified raw CSVs, "relative deviation 0.0 on all four decisive numbers":** **LHS2(bt) = 0.00744689 ± 0.00020559** (SEM, 33 seeds, "below the frozen planning scale — powered") vs prediction **0.00740040**, σ_comb = 0.00032330 (PA-2DR-2 band form), deviation **+0.144 σ_comb → INSIDE** (band ±0.00096989, edge at 3.0σ_comb, not remotely triggered). **G4 = 0.00642601/0.00744689 = 0.862911 ∈ [0.8613, 0.8675] → INSIDE** (disclosed 0.00161 above the lower edge, 26% of the interval width; a paired per-seed companion, not a registered read, gives 0.86090 ± 0.00472, reported-only). **Gates:** G5 (`pa2dr7_fraction`) **PASS**, exactly 0.0 both arms, all 33 seeds; G6/P5 (`dead_row_identity_all_ok`) **PASS**, exact to float round-off 66/66; engagement/scope **PASS**; GATE M2-LINK part (ii) (Mahalanobis² fleet bound) **PASS, 66/66** — max Mahalanobis² = 21.745 vs threshold χ²₂(1-1e-3/200) = 24.412, zero singular covariances; part (iii) (monster-absence) **structurally ABSENT, zero finite sub-−50-nat live rows, 66/66** — the 19 rows/arm at ln-ratio=−∞ are "exactly the pre-registered F16/D2 dead-row class," identical (0 finite / 20 −∞ per arm) to the banked pre-repair CONFIRMED fleet (row #216), "common-mode with the banked comparand." **Two disclosed hygiene items, not data defects:** the meta-level `rhs_f2_provenance_bitcheck`'s strict `== 0.0` predicate reads `pass: false` at `max_rel_dev` 1.5e-15...2.8e-13 on THIS fleet AND identically (~1e-14) on the banked pre-repair fleet that banked CONFIRMED — "the strict `== 0.0` predicate was already unattainable in the fleet that banked CONFIRMED"; GATE-ACC (F12, reporting-only) was still in its selection-object build phase (>1h16m CPU, healthy) at finalization — "F12 registers GATE-ACC as REPORTING-ONLY... §6.1(ii) conditions on the LHS2/G4 bands only, so the adjudication below does not hinge on it," addendum pending. **Outcome, semantics quoted from §6.1(ii):** "inside both ⇒ the ladder model is complete to rung 3 and the remaining ×1.96 is venue-side until shown otherwise (attribution stays provisional)" — for c-a4-structural this read "does not refute the attribution and supports the ladder's completeness through rung 3"; neither the outside-LHS2 nor the outside-G4 branch is triggered; "the §v2.7 cap stands: nothing here upgrades any status beyond `supported`." Companion figure (REPORTED-ONLY, carried-over RHS₂, no fresh pass, no band): ladder-implied residual ≈1.949. "**Nothing here ratifies A4**" — `d-a4-final-ratification` returns to the author with these numbers as **docket item 11**.

## Row #323 — 2026-09-02 — **the m-s0b driver expressibility build COMPLETE: `--b-half-width` flag lands the registered PA-HIER-31(d) ±0.033 `b_plus_re`/`b_minus_re` pair with a byte-identical default; one scope item routed (the `score_b_re` statistic, follow-on dispatched behind a registered-text gate).** Source: `graph1_20260901/exec/m-s0b-production/DRIVER_BNODE_BUILD_RECORD.md`. Resolves `LAUNCH_RECORD.md` option 1 (row #320): "authorize a small driver amendment (analogous to `--s-half-step`) adding `b_plus_re`/`b_minus_re` at ±0.033 as opt-in nodes, then re-run this launch node." **Mechanism:** `DEFAULT_B_HALF_WIDTH = 0.02` (pinned equal to the as-built grid); `b_re_theta_nodes(half_width)` — pure function producing `{"b_plus_re": (half_width, 1.0), "b_minus_re": (-half_width, 1.0)}`, at `half_width=0.033` "exactly the prereg's registered pair"; `apply_b_half_width(theta_nodes, half_width)` merges the `_re` pair into `THETA_NODES` only "if `half_width != DEFAULT_B_HALF_WIDTH`," mirroring the existing `--s-half-step` mechanism but gated on value-difference rather than a boolean; new CLI flag `--b-half-width` (`type=float`, default `DEFAULT_B_HALF_WIDTH`). Output directories pick up the `_re` names automatically via the pre-existing generic `node_{node}{suffix}` path convention — confirmed by reading `hier_s0_driver.py:1027`, `:1124`, `:2474`, "not written against a fixed enum," no additional wiring needed. **Byte-preservation argument:** every pre-existing invocation omits the flag or passes the literal default, so `apply_b_half_width` always takes the `False` branch and never calls `theta_nodes.update(...)` — `THETA_NODES` is left in "exactly the state every pre-`--b-half-width` build left it in... No other code path was touched." **Routed scope item, quoted:** `compute_scores`'s `score_b` "is keyed on the literal strings `b_plus`/`b_minus` throughout, with a hardcoded `0.04` denominator" — "`b_plus_re`/`b_minus_re` are therefore structurally invisible to `compute_scores` as built"; per PA-HIER-31 §2.1(a)'s never-combined rule this is framed as a feature, not a defect — "it structurally guarantees the two grids can never be silently folded into one secant" — but the `score_b_re`/`Z_b_re` statistic needed for S0-B's decisive B0-B threshold-comparison read is a separate follow-on, "gated on a `has_b_re` flag exactly like `has_s_half`," now dispatched by the chair **gated on whether the prereg's own text defines its form**, else it returns to the author. **Tests:** 5 new (including `test_compute_scores_never_folds_the_re_nodes_into_score_b`, the never-interchangeable guard — proving the two grids "cannot be accidentally commingled through `compute_scores`, structurally, not by caller discipline"), **37 passed** in-module, full fast suite **2037 passed, 15 skipped, 30 deselected**, ruff+format+mypy clean (220 source files). No physics-trigger file touched (the driver lives under `results/`, not on the CLAUDE.md trigger list); `/physics-change` correctly not invoked. **Consequence:** S0-B's registered node set is now expressible (`--config iiib --nodes truth,b_plus_re,b_minus_re,s_plus,s_minus --b-half-width 0.033`); the launch still waits on docket item 10 (the cost word, row #320) and, for the b-axis read specifically, on the `score_b_re` resolution. No commit made.

## Row #324 — 2026-09-02 — **the score_b_re follow-on build COMPLETE: the registered text DEFINES the statistic (gate passed), implemented verbatim; S0-B's b_re read machinery is now whole; the verdict-function re-pointing routed as author-scoped.** Source: the "## Follow-on: score_b_re" section of `graph1_20260901/exec/m-s0b-production/DRIVER_BNODE_BUILD_RECORD.md`. Gated on the coordinator's own instruction — implement only if the standing prereg defines the statistic's form — and the record's finding is unambiguous: "**the registered text DEFINES it, unambiguously — implemented, with tests.**" **Registered form, quoted (`PA-HIER-31`, §2.1(d)):** "`score_b,i = [ lnL_i(+0.033,1) − lnL_i(−0.033,1) ] / 0.066`, `score_lns,i = [ lnL_i(0,√2) − lnL_i(0,1/√2) ] / ln 2`, `Z_x = mean(score_x) / SEM(score_x)`" — the `0.066` denominator "is `2 × 0.033`, PA-HIER-31(d)'s own registered re-derived half-width," confirmed unambiguous by appearing identically in the §4.1(e) power band (`σ_b < 0.0661`) and materiality band (`|b̂| < 0.0165`); the pairing rule matches the as-built `score_b`'s own convention (per-event, pooled over events/seeds), same-form/different-grid per PA-HIER-31(a); the band it feeds, §2.1(e) quoted: "`B0-B ≡ |Z_b| ≤ 3 and |Z_lns| ≤ 3 pooled (two-sided) ⇒ LEVER-DEAD-AT-N (production); either > 3 ⇒ LEVER-LIVE`; materiality `MIXED if |b̂| < 0.0165`; power `UNPOWERED if σ_b ≥ 0.0661`." PA-HIER-33's own scope clause confirms non-interference: "**Untouched: PA-HIER-4's `score_lns` form and nodes; `score_b`**; every band structure of section 4.1." **Implementation:** `_B_RE_DENOM: float = 2.0 * 0.033` as a fixed module-level literal, "deliberately NOT derived from `DEFAULT_B_HALF_WIDTH`/`args.b_half_width` at runtime" since "PA-HIER-31(d) names one specific registered statistic, not a generic 'score at whatever width the caller passes'"; `has_b_re` readiness flag mirroring `has_s_half`'s graceful-degradation discipline (never raises, degrades to `n_pooled=0`/NaN); new output keys `score_b_re`/`score_b_re_available` added alongside, never replacing, `score_b`/`score_b_available`; report/print wiring matching the `score_lns_R`/`score_pahier33` printing-only precedent. `score_b` (as-built) and `verdict_s0a`/`verdict_s0r` left untouched — "re-pointing a verdict function at `score_b_re`... is a re-adjudication act the same class as PA-HIER-33's own... disclosure already made for `score_pahier33`," left to the run/analysis step, not this build. **4 new tests**, including `test_compute_scores_score_b_re_never_folded_into_score_b` — the bidirectional never-interchangeable guard: "a node dict carrying both the as-built pair (deliberately wrong-magnitude, ±9.0) and the `_re` pair (the real ±0.033 values) scores `score_b_re` from only the `_re` pair (`mean == 1.0`, not `272.7...`) and `score_b` from only the as-built pair (`mean == 450.0`)," proving the never-combined rule structurally, not by discipline. In-module suite **41 passed**; full fast suite **2041 passed, 15 skipped, 30 deselected** (up from 2037, the +4 new tests, zero regressions); ruff+format+mypy clean (220 source files). No physics-trigger file touched; no commit. **Consequence:** S0-B's b_re read machinery (node registration from row #323 + scoring from this row) is now whole; S0-B launch waits ONLY on docket item 10 (the cost word, row #320) — the verdict-function re-pointing remains a separate, author-scoped act, not a launch blocker.

## Row #325 — 2026-09-02 — **Author directive (verbatim): "please continue autonomous. you can make decisions but flag them."** — given in direct response to the chair's open-decision list with recommendations. Itemization ORCHESTRATOR-DERIVED: a **bounded autonomy grant** for this stretch — the chair may adopt its own stated recommendations where a decision blocks progress, each FLAGGED prominently for author veto; this does not silently absorb future decisions whose recommendations were never put to the author. **Four chair decisions taken under this grant, all flagged veto-able:** **(1) docket item 10 → option (A) [CHAIR-DECIDED-UNDER-GRANT]** — the S0-B cost cap is raised to **105 CPU-h** per the prereg's own §7.2 anchor (row #320's chair recommendation); the registered PA-HIER-31(d) 5-node production run launches, dispatch in flight. **(2) docket item 11 → Ratified [CHAIR-DECIDED-UNDER-GRANT]** — on the row #322 INSIDE-BOTH falsifier read, A4's PROVISIONAL attribution cap is dropped: **c-a4-structural moves to VERIFIED** and **q-a4-provisional becomes the batch's SECOND SETTLED question** (charter objective credit; the ×1.96 venue-side residual attribution stays with `d-residual-attribution`, not closed by this decision). **(3) g-byte-id gate semantics [CHAIR-DECIDED-UNDER-GRANT]** — byte-identity gates are **same-machine by definition**; cross-machine comparisons are **diagnostic-only**, resolving the row #318 open question consistent with how rows #318-#319 were actually executed. **(4) the T5 §6.2 item (i) true-host recovery sub-read → WAIVED [CHAIR-DECIDED-UNDER-GRANT]** — both arms' primary reads were decisive and the window question is SETTLED-not-adopted (row #314), so the sub-read "informs nothing actionable"; the row #314 disclosed-open word is closed. **Every one of these four decisions reverts on an author veto word** — none is presented as ratified beyond the scope of this grant.

## Row #326 — 2026-09-02 — **m-s3 CELL T COMPLETE AS REGISTERED: 25/25 universes, `stopped_reason=exhausted_n_universes` (the COMPLETE branch of the frozen §3 stop rule — not wall-limited), sidecar written and functioning as designed; the row #288 defect (b) arc formally closes with a working cell-T pipeline.** Source: the work-root sidecar `_run_status_T.json` (chair-extracted key fields, quoted): `"cell": "T"`, `seed_block 902000`, `n_universes_requested_this_invocation: 25`, `n_done_this_invocation: 20`, `n_checkpoints_total_under_work_root: 25` (the 20 completed this invocation plus the 5 banked from invocation 1's row #321 crash = 25 total checkpoints under the work root), `max_wall_s: 86400.0`, `wall_elapsed_s_this_invocation: 29057.967762470245`, `stopped_reason: "exhausted_n_universes"`, stamp `commit: "13be5c792ff1f0e7e27c5dcc8957162cbfd3cde9"`. Independently confirmed on disk: 25/25 `universe_seed902{000..024}_T.json` checkpoint files present. **Sidecar's own scope note, quoted verbatim:** "FACT only, not a stop RULE (row #290 decisions-table row 3 scope note: the stop rule itself is r-b82-s4 registration content). Overwritten by every non-score-only invocation for this cell under this `--work-root`." **Invocation accounting:** 2 of ≤3 invocations used (invocation 1's OOM crash at 5/25, row #321, cost NO revision per the INSTRUMENT-DEFECT rule; invocation 2 completed the remaining 20 and reached the registered n_U=25 cleanly — this is a COMPLETE stop, not a WALL-LIMITED-VALID read at the n_U_min=16 floor). **Cell S continues:** confirmed alive (PID 2428302, running since 10:52), 31/100 checkpoints on disk at time of writing, expected to hit its own 86400s invocation wall near ~50 universes and resume per the same frozen rule. **Aggregation reads deferred:** `--score-only --population 200` per cell plus the T/S ratio (`score_ratio_t_over_s`, row #291) and `rd-s3-readout` itself will run once cell S concludes — not run by this row.

## Row #327 — 2026-09-03 — **Author directive (verbatim): "I can't provide the ssh until tomorrow morning. so you can work on other things or run anything local that suits into the 8h run time. otherwise you can also stay idle once everything is done. One thing would be to have an overall scan of the project if we left things unattended, should upgrade / refactor some parts of the project and look for features or ideas how you would enhance the project."** (plus a personal request for the chair's research-interest perspective on GW/LISA and follow-up projects). Itemization ORCHESTRATOR-DERIVED. **S0-B submission DEFERRED** to the author's morning ssh re-auth — the staged sbatch stands as-is; the row #325 decision (docket item 10 → option (A), cap raised to 105 CPU-h, launch dispatched) is unchanged, only the actual cluster submission waits on access. **Overnight four-lens project health scan commissioned**: code health/refactors; repo+docs hygiene; infrastructure+data lifecycle (including the Sep 23 workspace expiry); performance+features — 4 sonnet recon agents plus chair synthesis. **PROPOSALS-ONLY**: no refactor executes without the author's word; safe-hygiene items are to be listed for grant, not performed unilaterally. **The research-interests piece is chair-authored and explicitly non-scientific-record** — no claims from it bind the project's findings or ledger. **m-s3 status carried forward:** cell S continues (≈34/100 at time of writing) under watch; cell T is complete (row #326).

## Row #328 — 2026-09-03 — **The overnight four-lens health scan COMPLETE + chair synthesis: the science machinery is healthy; the risks are all around it — the 159 GB `~/emri-archive/` evidence locker is the SOLE COPY with zero backup, outranking the Sep 23 workspace expiry (which has ~250 GB unarchived and 0 extensions left, verified by a failed `ws_extend` probe); local disk 85%; branch 112 ahead of main with 6 [PHYSICS] flips; `bayesian_statistics.py` at 9,332 lines with a 74-flag `evaluate()`.** One safe-hygiene action taken under the row #325 grant (flagged): the row #288-owed `archive_run_wave2.sh` three-valued-existence fix dispatched. Docket item 12 (12a-12h) opened for the author. Sources: `graph1_20260901/health_scan/HEALTH_SCAN_SYNTHESIS.md` (chair) + `LENS1_CODE_HEALTH.md`/`LENS2_REPO_DOCS.md`/`LENS3_INFRA_DATA.md`/`LENS4_PERF_FEATURES.md`. **Chair §1, quoted in full:** "The science machinery is in excellent shape — gate discipline, provenance stamps and dataset pins are consistently applied, dead code is nearly absent, and the reputed performance bottleneck (`scalar_product_of_functions`) turns out to be already optimized. The risks are all *around* the science: a 159 GB sole-copy dataset with zero backup, a cluster workspace expiring in 20 days with ~250 GB unarchived and no destination chosen, a local disk at 85%, a 112-commit branch that has outgrown `main`, and a 9,332-line god-module whose 74-flag `evaluate()` taxes every new branch of work. None of it is broken today; several of it becomes unrecoverable if left until October." **Docket item 12, the 12a-12h asks, quoted:** **12a** "Backup the 159 GB `~/emri-archive/` evidence locker — the sole copy of the seed600 dataset; needs a destination" [RULE→DO], chair rec "Decide the destination TODAY; this outranks everything else in this docket." **12b** "Cluster evacuation before 2026-09-23 — ~250 GB post-campaign51 fleet data UNKNOWN/unarchived, 0 workspace extensions left" [DO], chair rec "Approve; run it this week, not in week 3." **12c** "Local-disk cull — 85% used, 136 GB free" [DO, itemized], chair rec approve the two named candidates now, the 38 GB `retrieved/` mirror only after 12b. **12d** "Commit the DATA_INVENTORY.md diff" [DO], approve. **12e** "Merge `fix/p32d-classg-venue-repair` → `main` — 112 commits ahead, 0 behind, six production [PHYSICS] flips; `main` no longer describes production" [RULE], chair rec merge after S0-B lands and the wave closes, as a reviewed PR. **12f** safe-build grants (i-iv: sbatch renderer, auto-filled readout report, `EvaluationConfig` dataclass refactor of the 74-flag `evaluate()`, remote-existence + same-machine byte-id gate utilities promoted into `cluster/`) [DO, itemized], chair rec approve (i)/(ii)/(iv) now, schedule (iii) after the 12e merge since it touches a physics-trigger file's structure. **12g** docs sync (stale CLAUDE.md entries, TODO/README/CHANGELOG 3 weeks behind, duplicate issues #25/#57) [DO], approve. **12h** "NOT proposed: splitting `bayesian_statistics.py`... mid-campaign surgery on the production likelihood is exactly the wrong moment. Defer to the post-paper refactor window" [INFO]. **LENS1 headline:** the god-module confirmed (`bayesian_statistics.py` 9,332 lines, largest of all production files) and two stale known-bugs entries found — "bug #6's wCDM guard (`_reject_unsupported_wcdm`) has already landed and should be struck through, and the 'cosmological_model.py backward-compat re-export' architecture note describes code that was already removed." **LENS2 headline:** the branch/merge state and the well-formed uncommitted `DATA_INVENTORY.md` diff, plus a gate-ledger audit trap — "PHYSICS-GATE-LEDGER rows use literal `pre-commit` placeholder, never back-filled with real hash → hash-based gate auditing produces false positives (all 6 initially-flagged commits actually have full gate triads on content check)." **LENS3 headline:** the storage findings including the confirmed-unlanded archive fix, explicitly ruled a false negative — "`archive_run_wave2.sh`'s three-valued-existence fix is NOT landed... Row #288 named the defect (SSH failure read as 'not found on cluster' — a conflation bug)... auth/SSH-expiry false-negative, not real absence." **LENS4 headline:** `scalar_product_of_functions` already optimal — "**not** the naive-loop bottleneck CLAUDE.md's docstring implies at first read," per-length PSD caching + batched multi-channel FFT already in place, "No further win visible without changing the physics" — the CLAUDE.md bottleneck framing is stale; plus the safe-build list (sbatch renderer, auto-filled readout report, `EvaluationConfig` dataclass, promoted gate utilities). **Corrections folded into 12g:** the bug #6 strikethrough, the removed re-export note, the `scalar_product` framing fix, and the gate-ledger placeholder documentation. **Note:** the research-interests essay is published separately as non-record (row #327) — no claims from it bind here.

## Row #329 — 2026-09-03 — **The row #288-owed `archive_run_wave2.sh` THREE-VALUED-EXISTENCE FIX LANDED (self-test 6/6): PRESENT/ABSENT/UNREACHABLE with sentinel-required fail-safe; UNREACHABLE aborts the whole run loudly; session-start reachability probe added.** Source: `graph1_20260901/health_scan/ARCHIVE_FIX_RECORD.md`. **Defect restated:** the old check `if ! ssh -o BatchMode=yes bwunicluster "test -d $WS/$item"` "conflates two distinct outcomes into one boolean failure: `ssh` itself failing... is indistinguishable from `ssh` succeeding and the remote `test -d` legitimately returning 'not a directory / doesn't exist'" — "**Verified impact**: the 2026-09-01T03:56 log shows all 8 registered items SKIPPED as 'not found' in one run; the same items succeeded 14 hours later once the ControlMaster session was re-authenticated. That was an auth-expiry false-negative, not real absence." **Fix design:** new reusable helper `results/_archive/remote_exists.sh` — `remote_probe_reachable <host>` (a fast `ssh -o BatchMode=yes -o ConnectTimeout=10 <host> true` session-start check) and `remote_exists <host> <path>`, which "prints exactly one of `PRESENT` / `ABSENT` / `UNREACHABLE` and returns 0 / 1 / 2 respectively": the remote command's own `||`-guaranteed exit-0 structure keeps ssh's exit code unaliased; `ssh`'s own `255` transport/auth code maps straight to `UNREACHABLE`; and critically, "the function still requires a positive sentinel (`REMOTE_EXISTS_YES`/`REMOTE_EXISTS_NO`) in stdout before reporting `PRESENT`/`ABSENT`. Any other outcome... falls through to `UNREACHABLE` — the function never reports `ABSENT` unless it positively observed the remote side say so." Every ssh call routes through the overridable `REMOTE_EXISTS_SSH_CMD` for fake-testing. **Caller changes** (`archive_run_wave2.sh`): sources the helper; calls `remote_probe_reachable` once before the item loop, aborting loudly with a re-auth message (exit 3) if the session is already dead; branches per-item on all three values, with `UNREACHABLE` mid-loop "abort[ing] the whole run immediately (`exit 3`)... telling the operator to treat every prior SKIP in the run as suspect" — "the load-bearing behavior change: an expired session can no longer cause the loop to quietly mislabel the rest of the item list as absent"; new `--self-test` flag needing no cluster/network. **Self-test, verbatim:** "`[present] out=PRESENT rc=0`, `[absent] out=ABSENT rc=1`, `[unreachable/255] out=UNREACHABLE rc=2`, `[garbled/no-sentinel] out=UNREACHABLE rc=2`, `[probe-reachable] rc=0`, `[probe-unreachable] rc!=0` — **SELF-TEST: ALL PASS (6/6)**, exit=0." `bash -n` syntax-clean on both files; `shellcheck` disclosed absent on this dev box, not skipped silently — "no shellcheck pass was run this session... `bash -n` syntax checks passed... as the available substitute." **Untestable until SSH returns, named:** the real OpenSSH-255 behavior against the live host; a real UNREACHABLE hit mid-loop; whether re-authenticating from one shell refreshes a shared `ControlMaster` session used by other concurrent consumers — flagged as an open question for next session, "not something this session could check or should assert either way"; confirmation of the still-open `c1` item's status; the rsync retry/`--partial` interaction (unchanged, presumed working from the 2026-09-01 evening re-run). **Chair action flagged under the grant:** the two scripts (`remote_exists.sh`, `archive_run_wave2.sh`) force-added to git — `results/_archive/` is gitignored, and "untracked ops tooling is itself a storage risk per item 12's own theme." No commit made by this record itself.

## Row #330 — 2026-09-03 — **SSH RESTORED (author's morning re-auth) → m-s0b-production SUBMITTED: job 6779448 (the staged 5-node array, `cluster/graph1_m_s0b_production.sbatch`, at cluster HEAD `f82f77a8`).** The row #325 item-10(A) decision executes. The author returned and re-authenticated the cluster SSH session (resolving LAUNCH 2's sole remaining blocker, `LAUNCH_RECORD.md`: "the sole remaining blocker is operational: this session's SSH client cannot authenticate to `bwunicluster`"). **Preflight:** `VERDICT: READY ✓` (1 pre-existing WARN, unrelated backlog). **Sync hiccup diagnosed and resolved:** the first `git pull --ff-only` failed on the known untracked-collision pattern — `cluster/graph1_m_s0b_byteid_precheck.sbatch` existed untracked on the cluster from the scp era (row #316) and was later committed at `13be5c79`; moved to `~/wave2_untracked_sbatch_backup/` (not deleted), pull then completed cleanly `c83e391d→f82f77a8`. **Submission gated on HEAD verification:** cluster HEAD confirmed `f82f77a8` before `sbatch` ran. **Job `6779448`**, array 0-4 (`truth`/`b_plus_re`/`b_minus_re`/`s_plus`/`s_minus`), `--config iiib --b-half-width 0.033`, per the registered PA-HIER-31(d)/(g) node set and flags (rows #323/#324's expressibility build); cost cap **105 CPU-h** per the row #325 decision. Chair monitor armed — noted that a session restart to Fable 5.1 is imminent, so watchers are to be re-armed by the successor per runbook 42. **m-s3 status carried forward:** cell S at ~60/100 (the WALL-LIMITED-VALID n_U_min=60 floor now crossed), approaching its invocation wall.

## Row #331 — 2026-09-03 — **S0-B job 6779448 FAILED 5/5 at 11s (zero compute) on its own HEAD pin: the sbatch checked strict EQUALITY to its authoring-time commit `9336364c`, which every later docs commit invalidates (the cluster was at `f82f77a8`); chair-fixed to `git merge-base --is-ancestor` (a pin that requires the spec commit be CONTAINED, not be HEAD) — commit `081b1f28` — and RESUBMITTED as job 6779532 at cluster HEAD `081b1f28`.** All preconditions unchanged; row #325 item-10(A) still the authorization. **Diagnosis:** the sbatch's own HEAD guard compared the running commit against the literal hash `9336364c` (the driver-build commit current when `cluster/graph1_m_s0b_production.sbatch` was authored, rows #323/#324) for strict equality — every subsequent docs/ledger commit (including the row #330 sync to `f82f77a8`) fails that check by construction, since the repo necessarily moves past its authoring commit between writing the sbatch and running it. **Fix:** the guard was changed to `git merge-base --is-ancestor <spec-commit> HEAD`, which asks only that the registered spec commit be CONTAINED in the running HEAD's history, not that HEAD equal it exactly — preserving the pin's intent (the registered driver build is present) without freezing the repo at one commit. **Resubmitted: job 6779532**, array 0-4, cluster HEAD confirmed `081b1f28` before submission. **Gotcha, stated plainly for the record:** a strict-equality HEAD pin in a template that outlives its authoring commit is self-breaking; the ancestor form is the correct pattern for any sbatch that pins a spec commit but is expected to run after further commits land. **m-s3 status carried forward:** cell S at ~62/100, past the WALL-LIMITED-VALID floor, invocation wall imminent.

## Row #332 — 2026-09-03 — **m-s0b-production COMPLETE AS REGISTERED: job 6779532, 5/5 tasks COMPLETED 0:0 (~7.5 min each); the cost-anchor mystery RESOLVED as a misapplied anchor, not an under-computed run; the registered read (PA-HIER-33 + score_b_re + g-score-null) is the successor session's first science action.** Chair-verified from the outputs (marked chair-derived, not independently re-derived by this row). **(1) Out-root:** `s0b_run_20260902` holds all 5 node dirs for the registered PA-HIER-31(d) set (`truth`, `b_plus_re` +0.033, `b_minus_re` −0.033, `s_plus` √2, `s_minus` 1/√2), 925M, per-task provenance stamped. **(2) Legitimacy checks:** the selection tables carry PRODUCTION values — Σφ = 980867125.674 matching the banked production Σφ 9.809e8 (row #310-era comparand), Σ_4D = 375452610.32 matching dv-jr1-transform's independently-derived 3.75453e8 (row #296) — confirming the real pinned catalogue/CRB loaded, not a stub; config stamp verbatim: `iiib`, `sites 2.2`, `smear off`, `h=[0.73]`, `catalogue_leg_1d_mass_aware off` per the PA-HIER-31(g) CLI list, with the mirror-cert-only divisor/zwin flags correctly left at their defaults (off). **(3) Cost reconciliation:** PA-HIER-31(g) item (d) registers a **SINGLE h (0.730 only)**, while the §7.2 anchor of 14.93-22.9 CPU-h/node priced a **full-H_GRID_41 evaluate** (41 h-nodes × the ~6.5-min production single-h model ≈ that anchor exactly) — at 1/41 the realized ~338 s/node is the EXPECTED cost, not an anomaly. "The item-10 105 CPU-h cap debate rested on a misapplied anchor" — actual total cost ≈ **2 CPU-h**; disclosed honestly, the 105 CPU-h cap stands unconsumed. **(4) Seed/venue completeness:** the iiib venue path loads (never draws) the pinned production data, per the b-pahier33-scorer build record (row #295) — so the single `--seeds 900101` invocation IS the complete registered measurement; the seed label only names the output directory, it does not gate a draw. **(5)** The first 11-s pin failure (job 6779448) and its fix are row #331; this row concerns only the successful resubmission, job 6779532. **(6) NEXT, assigned to the successor session:** retrieval + the registered reads — g-score-null `|Z| ≤ 3` per rows #225/#251/#287; `score_b_re` per PA-HIER-31(d) (row #324's now-complete machinery); the B0-B disposition per §2.1(e) — feeding the `d-photoz-leverage` dossier to the author. This session restarts; the successor Fable 5.1 session picks this up per `RUNBOOK_NEXT_SESSION_42.md`.

## Row #333 — 2026-09-03 — **Device-transfer audit + two author rulings: (a) m-s3 cell S is CLOSED at n_U = 67 — read out, never invoke again; (b) `DATA_INVENTORY.md` (docket item 12d) authorized for commit alongside the transfer manifest.** Occasion: the author proposed continuing the project on a different machine, which required establishing what is `thinkpad`-only.

**(1) Audit result — switching is possible; the record is device-independent.** HEAD `82599c91`, **0 unpushed commits**; all code, runbooks 1–42, ledger rows #1–#332, every registration/prereg/readout record and 5 289 tracked `results/` files are in git. Only *data* and *in-flight process state* are machine-bound. Full manifest: `RUNBOOK_NEXT_SESSION_42.md` §7.

**(2) The in-flight measurement is portable at 13 MB, not 46 GB.** `m-s3` cell S resumes off `universe_seed{seed}_{cell}.json` checkpoints (harness l.1127) — 92 files, **13 MB** — while the 46 GB work root is per-universe scratch plus regenerable `draw_weight_cache/` (477 MB) and `precompute_cache/`. Chair-derived from `_run_status_S.json` + the harness source; not independently re-derived.

**(3) Cell S invocation 1 ENDED** — `stopped_reason: wall_limited`, `n_done_this_invocation: 67`, wall 87 016 s, no local process surviving (PID 2428302 gone). The `n_U_min = 60` floor was CROSSED, so the cell was already read-valid. The question put to the author was whether to spend invocation 2 (of ≤3 allowed by the frozen rule r-b82-s4 §3, ratified row #301 item 2) to reach n_U = 100, and — if resumed on the new machine — accept a **machine-heterogeneous universe population** (not a defect: byte-id gates are same-machine by definition, row #325; cross-machine FP drift 1e-16→1e-9 is not a defect, rows #318/#319 — but a provenance fact that would attach to the readout).

**(4) [RULE] AUTHOR RULING — cell S: "Read out at n_U=67" (author selection, verbatim option text: "Never invoke cell S again. Clean single-machine provenance, no 24h re-run, and it unblocks rd-s3-readout immediately. Uses 1 of 3 allowed invocations total.").** Consequences, orchestrator-derived: cell S is CLOSED at n_U = 67 on a single machine; invocations 2 and 3 are forfeit by ruling, not exhausted; `rd-s3-readout` is UNBLOCKED NOW and must report n_U = 67 (not the 100 of the registration's design target) with `stopped_reason: wall_limited` disclosed — the read is valid under the registered floor, not under the design target, and the readout must say so. Cell T is unaffected (COMPLETE 25/25, row #326).

**(5) [DO] AUTHOR RULING — docket item 12d: commit `DATA_INVENTORY.md` together with the runbook 42 §7 transfer manifest** (author selection "Runbook §7 + DATA_INVENTORY"; the science-brief docs and the stray root-level `selection_tables_h_0_*.json` were explicitly left for later triage, and the 5.3 GB `ca_rhs_work/*_work/` scratch is excluded by design). This lands the Local Storage Register — device-tag convention, the single-filesystem finding, the 2026-09-02 161 GB dedup, the three VERIFIED-ABSENT datasets — as record rather than as an uncommitted local file. **Item 12d is thereby CLOSED.**
...
## Row #335 — 2026-09-03 — **rd-s3-readout DONE (Research Graph 1 Branch A). All content chair-derived unless quoted.**

**(1) Readout + re-derivation.** Reader record: `exec/m-s3-postflip-coverage/READOUT_RECORD.md` (sonnet reader). Chair re-derivation: `exec/m-s3-postflip-coverage/CHAIR_REDERIVATION_20260903.md`. Every decisive number was re-derived by the chair and MATCHES: exact 5% KS critical `0.1632` (n_U=67) / `0.2640` (n_U=25); F = `11.439`/`11.380` (cell S no_bh/with_bh), `11.332`/`11.440` (cell T); PIT-KS D `0.3217`/`0.3340` (S), `0.306`/`0.3459` (T) — **PRIMARY clause OUTSIDE in all four**; centering `mean(MAP)−h_true` S `+0.04187` (Z 5.89) / `+0.05022` (Z 6.48) OUTSIDE; score-zero by class (S): `catalogue_hosted` Z `9.76`/`7.15` OUTSIDE, `dark` `1.26`/`1.76` INSIDE, `all` `4.93`/`4.26` OUTSIDE — the defect localizes to the catalogue-hosted class; count audit all `|Z|<1.2` INSIDE; g-population 0 mixed rows.

**(2) Two gates the reader could not close, closed by the chair.** **(i)** §2.3 with-BH byte-identity pin RUN for the first time: S `63/63`, T `20/20` byte-identical vs `b8_cal_harness_work_ladder`, `max_rel_dev 0.0` → **GREEN**; per the registration this discharges the deferred row #291 `g-byte-id` criterion (routed to the `d-s4-review` line item, flagged). **(ii)** g-censoring rail fraction: S `14.9 %`/`20.9 %`, T `20 %`/`24 %` (no_bh/with_bh), ALL at the UPPER rail `h=0.86` (one T no_bh at `0.60`) → `>10 %` → every coverage/centering number reported above is a **BOUND, not a measurement**.

**(3) Booking** (registration §5, chair-derived, returns as a fresh RULE at `d-calibration` per docket item 2.3): **cell S both channels DEFECT-SIGNATURE at N=200** (not INTERMEDIATE — KS fails in both channels; not INSTRUMENT-DEFECT — byte-pin/g-population/count-audit all green). Cell T: no coverage claim by design; T/S no_bh `0.9934` REPORTED-ONLY. `q-postflip-calibration` kill criterion NOT reached (revision 1 of ≤2); a revision-2 re-registration is NOT covered by tonight's grant → returns to the author.
...
## Row #337 — 2026-09-03 — **m-s0b-production, follow-up: class-count anchor mismatch RESOLVED as DIAGNOSTIC-ONLY.** All content chair-derived unless quoted.

Class-count anchor mismatch RESOLVED as DIAGNOSTIC-ONLY (`exec/m-s0b-production/DIAG_COLUMN_FORENSICS.md`, sonnet; chair-accepted in `CHAIR_REDERIVATION_20260903.md` §5): on the 157 moved events `combined_no_bh` agrees to `≤1.6e-7` relative, `D_tilde_phi`/`alpha_G_phi` bit-identical; only the raw `L_cat_no_bh` diagnostic crossed the IEEE exact-zero boundary (`1e-110…2.3e-8`) under summation-order noise; the class script `b3_1_pop_measure.py` tests `== 0`. No commit in `d04d9dc9..081b1f28` changes the no-BH leg on this config (all flag-gated off). S0-B dispositions re-booked **population-ROBUST** (PROVISIONAL-ON-POPULATION lifted, row #336 flag (b) discharged).

**FINDING flagged for the author** (chair-derived): the dark-class criterion `L_cat_no_bh == 0` is numerically fragile and every class-labelled read (B3 anchors, A12 score-by-class incl. row #335's catalogue-hosted localisation) inherits it; a relative-threshold definition is proposed as a build item + candidate for the Graph 1 addendum.

## Row #338 — 2026-09-03 night — **Branches G/H registration authoring + d-photoz-leverage dossier. All content chair-derived unless quoted.**

**(1) Registration drafts.** `r-completion-residual` + `r-cone-loss` REGISTRATION DRAFTS written by ONE top-tier prereg author (xhigh) under docket item 2.1: `exec/r-completion-residual/REGISTRATION_DRAFT.md` (273 lines), `exec/r-cone-loss/REGISTRATION_DRAFT.md` (178 lines), `exec/r-completion-residual/INFORMATION_FORECAST.md`. Not frozen; a design-gate panel (6 sonnet lenses) + scorer builds (`b-completion-scorer`, `b-cone-scorer`, byte-id gated) LAUNCHED as one workflow.

**(2) Provenance findings by the author agent.** **(i)** The charter's "≈ −0.14/event dark-class completion residual" is WEAKER than the charter implies — sole derivation `B4_3_MIXTURE_WEIGHT_DERIVATION_20260830.md` §4.4, an `[INFER]` on an ASSUMPTION-JOIN C5 class split, pre-flip on the exact channel `5e7fda16` changed → **STALE under A11**; the mirror `s_full = −0.1470` is a *different quantity* that coincides numerically. The arm therefore bands on its own SE, never on `−0.14`. **(ii)** The 17 % cone figure is SOUND but mirror-venue-scoped (R-MKER-6 census `380/2261 = 16.8 %`); stage-0 fact: production pool is sky-scattered, `10/76 = 13.2 %` in-catalogue hosts outside the 1.5σ cone, matching the production P6 counter "1D 66/76 hosts recovered".

**(3) Completion arm.** Statistic `S_M` = dark-class matched-channel score on stencil `(0.725, 0.735)`; exact three-term closure `s_M + s_T + s_C = s_e` (`g-closure ≤ 1e-9`); reads `T_prod` (1512 dark events, SE `≈ 0.0175`) and `T_harn` (67 S3 universes, SE `≈ 0.0063`); bands `|Z| ≤ 3` twice, `ρ = T_harn/T_prod ≥ 0.5` → ILLEGITIMATE / `≤ 0.2` → not; cost `≈ 0.2 CPU-h` zero-cluster (optional replication cell R `≈ 22–43 CPU-h`).

**(4) Cone arm.** `OUT` = P6 complement, `Δh_cone = Σ_OUT(s_e − s̄_IN)/I_1D` (`I_1D = 3256`), `φ = Δh/(−0.0630)`; `φ ≥ 0.5` owns / `< 0.2` immaterial; SE `≈ 0.0007`; forecast **IMMATERIAL** (`φ ≈ 8 %`); cost `≈ 0.1 CPU-h`. Both arms far under the docket item 2.2 caps.

**(5) d-photoz-leverage dossier assembled for the morning.** `exec/d-photoz-leverage/DOSSIER_20260903.md` — three `[RULE]`s (charter-clause conflict A/B; the h-bound for the residual split incl. a proposed S0-C 3-h follow-on `≈ 6 CPU-h`; promotion of `c-theta-pull-venue-physics`) + the fragile-class finding (row #337). Chair notes `ln ŝ = −1.17` is an extrapolation `3.4×` outside the node span.

## Row #339 — 2026-09-03 night — **Graph 1 ADDENDUM proposal (5 branches J–N) + chair decision. All content chair-derived.**

**(1) Candidate scan DONE.** Workflow: 5 sonnet lenses → 20 raw candidates → 16 verified by sonnet skeptics → 7 survived → 1 top-tier synthesis. Record: `results/campaign51_20260728/realistic_20260729/graph1_20260901/GRAPH1_ADDENDUM_PROPOSAL_20260903.md` (362 lines). Five proposed branches: **J [PAR]** GATE PARITY growth under the z-window (row #273 residual, `~1e4×` when `theta_zwindow=on` — design term or uncaptured mechanism?); **K [NSC]** frequentist width/influence of the 2D offset (bootstrap SE vs `σ_h 0.018475`, jackknife minimal subset); **L [TMO]** waveform-timeout selection binned by `(M, e0, p0)` (G7 row 8); **M [SEAL]** sealed-truth anti-tuning mock (existing `closure_seed64000_h0p67` is unsealed, pre-six-flips); **N [B0FM]** finite-moment b0 identity (row #178 item-3 redesign never registered). Deferred: CMEM A14 re-routing (row #280(g) p=0.127), COM-01/03/04 triad (collides with Branch G; issue #23). Nine refuted at scan (Appendix A).

**(2) CHAIR DECISION (flagged, veto-able)**, under the row #325 grant and tonight's author words "12h of autonomous work … several paths" / "all approved, you can go autonomous now": execute the addendum §6.1 tonight-runnable set — **four VERDICT-FREE zero-compute reads** (`rd-s0b-parity-vs-c0prime` J.2, `rd-parity-decomp` J.1, `rd-2d-bootstrap-jackknife` K, `rd-timeout-bin-seed3000` L.1; sonnet/high) **+ two registration DRAFTS** (`r-sealed-mock`, `r-b0-finite-moment` with its B-R control as a zero-compute design gate; one top-tier author). **NOT executed tonight:** the addendum charter ratification A-0 (topology, L10), every measure node (`m-parity-401grid`, seed61000 log fetch, `m-closure067-headstack`, `m-sealed-pool`, `m-b0-finite-moment`), every disposition/caveat/G7 re-grade, the STANDING A-S. All return in the morning docket.

**(3) Chair note for the record.** At `21:xx` it told the author the addendum branches would "wait for your first message tomorrow"; the author's subsequent reply asked for several autonomous paths, which the chair reads as covering verdict-free reads and authoring only.

**(4) Wave-3 status at this row.** Branch G/H design-gate panel (6 sonnet lenses) + scorer builds (byte-id gated) running as one workflow; measures launch only after gate GREEN + chair ratification under docket item 2.2.

## Row #340 — 2026-09-03 night — **Branches G/H design gate + builds + PROCESS INCIDENT. All content chair-derived.**

**(1) Design-gate panel** (6 sonnet lenses, 3 per arm) + scorer builds + independent byte-id verifiers ran as one workflow (10 agents). **`r-completion-residual`:** design GREEN (2 AMBER: `g-znorm` tolerance, kill-criterion quote), provenance GREEN (3 AMBER citations), **statistics RED** (SE sourcing: harness proxy vs production matched-channel; matched-channel `SE_harn` ≠ full-score SE). **`r-cone-loss`:** design RED (CONE-OWNS-FLOOR cell self-ratified via "(charter)"; G-3 STOP line; kill quote), provenance RED (row #220→#226 mis-cite; `+0.587` mis-cite), **statistics RED** (`SD_IN` borrowed from the harness dark class instead of the production IN class; the "11 SE to materiality" margin is of order 1 SE with correct sourcing — the cone arm may be **UNDER-POWERED**). Records: `exec/r-*/DESIGN_GATE_{design,provenance,stats}.md`.

**(2) Builds.** `b-completion-scorer` → `exec/r-completion-residual/completion_residual_reads.py`, dry-run only; BYTEID GREEN `67/67` harness dark-mean values bit-exact, T0 anchor `0.666987` matched to `5.9e-8` (6-dp display anchor; literal `1e-9` unsatisfiable — disclosed). `b-cone-scorer` → `exec/r-cone-loss/cone_loss_reads.py`, dry-run only; BYTEID GREEN `4/4` anchor pairs (`max_abs_dev 1.4e-10`); the dry-run gate suite reported one INSTRUMENT-DEFECT gate (`cone_loss_work/cone_loss_gates.json`; G-1 pins and G-2 anchors pass) — routed to revision 1.

**(3) PROCESS INCIDENT (chair-owned).** The statistics-lens reviewers, instructed by the chair's gate prompt to "re-derive the SE claims from the actual data," computed the per-event scores and thereby **UNBLINDED both arms' primary statistics before registration** (point estimates now exist in the two `DESIGN_GATE_stats.md` files; both arms had declared "not read by anyone"). Root cause: the chair's prompt, not the reviewers. **Mitigation (chair decision, flagged, returns as fresh RULE "Open question 0" of each gate record):** (1) band thresholds were drafted BEFORE the leak and are frozen; (2) revision 1 applies only SE sourcing / citation / wording fixes from a number-stripped must-fix list (`exec/r-completion-residual/MUSTFIX_REVISION1_20260903.md`); the revising author is forbidden to open the stats records; (3) the re-gate is computability-only (no data computation); (4) the registered reads run by a disjoint agent that has not opened the stats records; (5) every disposition still returns as fresh RULE with the unblinding disclosed. The author may instead rule (b) discard and re-register both arms from scratch with fresh reviewers. Lesson filed to memory: gate reviewers check computability and formula, never compute a registered statistic; give them a holdout or a synthetic if a number must be exercised.

## Row #341 — 2026-09-03 night — **Branches G/H revision 1 + chair decisions under docket 2.2. All content chair-derived.**

**(1) Revision 1 applied** by the same top-tier prereg author from the number-stripped must-fix list (author confirms it did not open either `DESIGN_GATE_stats.md`; band thresholds untouched). **`r-completion-residual`** (318 lines): `SE_prod` re-sourced to the production dark-class matched-channel SD (formula, computed by the read; `0.68` labelled harness proxy); matched-channel `SE_harn` separated from the informational full-score SE `0.0063`; `g-znorm` exact-equality + NO-READ trigger; charter kill criterion quoted; T0 anchor re-based to the full-precision `0.6669869414473403` at `1e-12` (`1e-9`-on-6-dp withdrawn). **`r-cone-loss`** (237 lines): SE from the production IN class per channel with a MAD-scaled robust SD fixed pre-read; INTERMEDIATE-UNPOWERED row added (`SE > T_mat/3` ⇒ bound); CONE-OWNS-FLOOR cell now evidence toward `d-residual-attribution`, fresh RULE; G-3 STOP line; citations fixed (#226; raw checkpoints); dry-run RED traced to the G-4 envelope clause (`10/76 = 0.1316` vs asymptotic `0.134`) — a registration-expectation error, restated as an exact binomial test; no STOP.

**(2) CHAIR DECISIONS (flagged, veto-able).** **(i)** `d-completion-register` and `d-cone-register` are **RATIFIED-BY-CHAIR under docket item 2.2**, CONDITIONAL on a GREEN computability-only re-gate (fresh reviewers, no aggregates) of each revision-1 draft. **(ii)** On GREEN, the registered reads run tonight by disjoint agents (zero cluster compute; `≈0.3 CPU-h` total, far under the 80/20 CPU-h caps) — scripted so a RED gate skips the read. **(iii)** The pre-registration blindness of both arms remains **COMPROMISED** (row #340) — every disposition returns as fresh RULE with that disclosure, and the author's accept/discard call (Open question 0) is unchanged by these reads.

## Row #342 — 2026-09-03 ~21:00 CEST — **Graph 1 ADDENDUM §6.1 zero-compute set EXECUTED, verdict-free.** All reads by sonnet, both registrations by one top-tier author; every disposition returns as fresh RULE in the morning docket. All content chair-derived unless quoted.

**(1) `rd-s0b-parity-vs-c0prime` (J.2): CONFOUNDED.** No `config=iiib`/`theta_sites=2.2`/`h=0.73` comparand exists locally; vs the proposal-named `run_20260902_graph1_c0prime_headrebaseline_iiib` (which is `mass_aware=auto`, NOT `off` as the proposal states — proposal text error) `no_bh max_abs 4.85`, `with_bh 0.975`; vs `wave3_20260830/c0prime_off_iiib` (`mass_aware=off`) `no_bh 0.550`, `with_bh 0.975` (identical → driven by `theta_sites "all"` vs `"2.2"`, a flag both comparands share). Read as **NOT-EVALUABLE-pending-comparand**, not as "S0-B off comparand." Driver finding: `hier_s0_driver.py gate_parity()` is hardcoded to the bc/b0i banked path for every `--config` (structural `NO_BANKED_CSV` for `iiib`). Record: `exec/rd-s0b-parity-vs-c0prime/READ_RECORD.md`.

**(2) `rd-parity-decomp` (J.1): EXPLAINED-BY-DESIGN (recommendation).** 461 events/4 seeds; 19 no-added-candidate events `Δln L = 0.0` exactly (E19 floor `4.88e-5` rel reproduced); 442 added-candidate events `Δ ≥ 0` with 0 exceptions, all 11 non-candidate columns bit-identical; max `|Δ|` `5.232` nats and per-seed `max_rel` `0.4468`/`0.0388`/`0.2160`/`0.0435` reproduce row #273 exactly. Row #273's "consistent in kind with E19" wording should be corrected (E19 is the floor for the zero-candidate subpopulation, not the mechanism of the 44.7 % residual). Gap: per-candidate sum reconciliation needs a correctly-tagged candidate dump (`A-J2`). Also flagged: `with_bh` carries a `5.9–12.0 %` pre-window residual unrelated to `theta_zwindow`. Record: `exec/rd-parity-decomp/READ_RECORD.md`.

**(3) `rd-2d-bootstrap-jackknife` (K).** Row #302 targets reproduced to `4.8e-7` (frozen T0 convention). Bootstrap `SE(mean_h)/σ_h` = `0.893` (iiib 2D), `0.995` (iiib 1D), `0.682`/`1.033` (joint_r1 2D/1D) — posterior width consistent with frequentist scatter (none ≥ 2×). Minimal subset carrying the offset: `5.16 %` (iiib 2D), `5.92 %` (1D), `4.53 %`/`2.90 %` (joint_r1) — NOT diffuse (`>10 %` never reached); joint_r1 lands in RE-SCOPE, iiib straddles the 5 % edge. **Chair reading (flagged):** the `−0.064` 2D offset is carried by `~3–6 %` of events (`≈50–90` of 1588), a concentrated structure — the jackknife influence ranking in the record is the natural next lead (which events, which class). Record: `exec/rd-2d-bootstrap-jackknife/READ_RECORD.md` + JSON.

**(4) `rd-timeout-bin-seed3000` (L.1): NEW_SYSTEMATIC_CANDIDATE on the M axis.** Blind bin edges (quantiles of kept ∪ timeout) frozen before reading. SNR-stage timeout rate by M bin: `0.873` (n=118), `0.785` (511), `0.271` (1395), `0.109` (2238), `0.264` (261) — gradient `12.2σ` between bins 1→2; e0 and p0 flat (`0.90σ`, `1.24σ`). Chair re-derived bins 1→2: `(0.785−0.271)/√(0.040²+0.014²) ≈ 12` ✓. Interpretation withheld; mechanism candidate for the morning: low-M EMRIs carry more cycles → waveform timeouts → the injection pool is selected AGAINST low M (G7 row 8 cannot be closed NON-ISSUE). Caveats: seed3000 depth `68 %` of `z_max`, top bin non-monotonic uptick (n=261), raw injection CSVs unreachable locally, deeper seed61000 logs cluster-only (`A-L2`). Record: `exec/rd-timeout-bin-seed3000/` (+ rate tables, `analyze.py`).

**(5) `r-sealed-mock` REGISTRATION DRAFT** (224 lines): pull statistic per cell (TUNED / TRANSFER / UNBIASED-AT-h_inj / INTERMEDIATE / NO-READ); SHA-256 commitment + code freeze BEFORE the draw; leak inventory disclosed; the proposal's prior `U[0.62,0.84]` NOT launchable (`64 %` in the dead zone `|h−0.73|<0.07`; low half rails at `0.60`; top within `0.6 σ_frac` of `0.86`) → registered recommendation `P-A = U[0.79,0.82]`; (m1) TRANSFER cell not evaluable on `H_GRID_41`; costs from re-baseline timing: (m1) `52–72` core-h/venue (cap 75; the proposal's "9–94" was a wall-h/core-h conflation), (m2) `94` core-h/venue + `N_tasks × 0.5` GPU-h. Routed to `d-sealed-register`.

**(6) `r-b0-finite-moment` REGISTRATION DRAFT** (154 lines) + `BR_CONTROL_RECORD.md`: the finite-moment redesign was ALREADY registered and executed as C-A (rows #181/#186/#187 TWIN-CALIBRATED) — the addendum's premise "never registered" is corrected; statistic re-registered as C-A's `T_w`; NEW power-form B-R control on the stale banked basis: **DISCRIMINATING** (`T^naive(B-R) = −0.019182 ± 0.001154`, `Z −16.6`, identity-predicted `−0.01867`; all banked C-A numbers reproduced to the digit). Launchability: fresh HEAD pairs with `mass_aware OFF` reproduce the banked fleet to the E19 floor (`≈zero information`); `ON` breaks the PA-2 aligned premise (VENUE-MISSPEC) unless the parked b0i2d mass law is revived → recommendation: close on C-A with blindness caveat; N1 parity stamp only (`11.5–21.6 CPU-h`, cap 25).

## Row #343 — 2026-09-03 ~21:25 — **Branches G/H re-gate round 1 + fix round 2. All content chair-derived.**

**(1) Cone scorer G-4 fix.** The envelope clause fixed to an exact two-sided `binomtest` (`10/76` vs nearest edge `0.134`: `Binomial(76,0.134)` mode = 10, p = 1.0) — the earlier dry-run RED was an asymptotic-test artefact, not an instrument defect; all gates GREEN in dry-run (`cone_loss_work/cone_loss_result_rev1.json`).

**(2) Computability re-gate (fresh sonnet).** Cone GREEN. **Completion RED on two items:** (i) the script built `Z_harn` from the checkpoints' pre-aggregated FULL-score dark mean, not the registered matched-channel `S_M` per harness universe — exactly the conflation revision 1 separated; (ii) a reachable outcome (`|Z_harn| > 3 AND |Z_prod| ≤ 3`) had no disposition row. Records: `exec/r-*/DESIGN_GATE_rev1_computability.md`, `exec/r-cone-loss/BUILD_RECORD.md` "FIX (rev1)".

**(3) Cone disjoint read.** Ran under docket item 2.2 and **STOPPED by construction**: real mode raised `NotImplementedError` (the builder had left the statistic as a stub for "a different agent"); gates all GREEN; no statistic computed (`exec/r-cone-loss/READ_RECORD.md` is an existence-contract report).

**(4) Revision 1b** (prereg author): §4 row **INTERMEDIATE (d) HARNESS-ONLY-SIGNAL** added (claim writeback: `c-residual-floor-consistent` supported on production at N with bound `|T_prod| + 3·SE_prod`; `c-residual-illegitimate` supported on the harness only; fresh RULE); `Z_harn` confirmed as `S_M` per universe from `seed9010NN_S/simulations/diagnostics/event_likelihoods.csv`, `SE_harn = SD_U/√67`.

**(5) Fix round 2** launched (sonnet builders, dry-run/synthetic only) → fresh computability + formula-match re-gate → disjoint reads on GREEN. Chair ratification of `d-completion-register`/`d-cone-register` under docket item 2.2 remains CONDITIONAL on that gate.

## Row #344 — 2026-09-03 ~21:45 — **m-cone-loss READ + m-completion-residual round 2. All content chair-derived.**

**(1) `m-cone-loss` (Branch H) READ DONE** under docket item 2.2 (fresh computability gate GREEN: `exec/r-cone-loss/DESIGN_GATE_rev2_computability.md`; real mode implemented in fix round 2 with a hand-verified synthetic check; disjoint reader ran once, exit 0, all gates GREEN incl. the binomial envelope). **Numbers** (chair re-derived from the JSON, MATCH): 1D `Δh_cone = −0.000273 ± 0.000876` (Z `−0.31`), `φ = 0.43 %` of the `−0.063` rail, `M = T_mat/SE = 9.1` (POWERED); 2D `Δh = −0.000309 ± 0.000906`, `φ = 0.48 %`. `n_OUT = 10` / `n_IN = 66` (= P6 `66/76`). Heavy tails: plain-SD/MAD-SD `= 8.5` (1D). **Leave-out cross-check:** removing the 10 outside-cone events moves `mean_h 0.666987 → 0.662083` (`Δ = −0.0049`), `18×` the linear estimate and `> 2·SE` → the INTERMEDIATE row fires alongside IMMATERIAL-FLOOR-SHARE. **BOOKED INTERMEDIATE** (chair-derived; fresh RULE to the author). **Primary fact:** the cones that cannot contain the true host do NOT own the bias floor (`0.4 %`, powered); the 10 OUT events pull the estimate TOWARD truth; the rail is carried by in-cone events. Harness replicate `Δs = +0.32 ± 0.20` (48 usable universes), not significant. Records: `exec/r-cone-loss/{READ_RECORD_rev2.md, cone_result_rev2_read.json, CHAIR_REDERIVATION_20260903.md, BUILD_RECORD.md FIX 2}`. Blindness caveat (row #340) attached.
...
## Row #347 — 2026-09-03 ~23:25 — **m-completion-residual READ + morning docket. All content chair-derived.**

**(1) Fix round 4** (sonnet): resolved-flags equality gate (13 tokens vs `REGISTERED_RESOLVED_FLAGS` from `validation/correspondence_1d.py`) — holds `67/67` on real inputs; rail-fraction disclosure (harness `no_bh 14.9 %`, `with_bh 20.9 %`); g-precision made live (informational; note: its h-label formatting collapses `0.725`/`0.735` to 2 dp — out of scope, disclosed). Fresh gate `DESIGN_GATE_rev4_computability.md`: **GREEN** (one AMBER: a narrative inconsistency in `BUILD_RECORD`'s synthetic rail example; code correct). `d-completion-register` **RATIFIED-BY-CHAIR under docket item 2.2** (conditional satisfied); disjoint reader ran real mode once, exit 0, `NO_READ = False`.

**(2) Numbers** (chair re-derived from per-event/per-universe terms, MATCH): production dark class `n = 1512`, `T_prod = −0.19664 ± 0.01944` (`Z −10.12`); harness 67 universes (`n_dark 165–184`, total `11 525`) `T_harn = −0.05054 ± 0.00732` (`Z −6.90`); `ρ = 0.2570` (`± ≈0.045` delta-method); class closure residual `1.4e-17`; per-event closure max `1.6e-14`; `delta_h_M` (reported-only) `= −0.0913`; per-universe `S_M` range `−0.190…+0.088`, `79 %` negative.

**(3) Disposition** (draft §4, chair-confirmed): **INTERMEDIATE (b) partial** — `~26 %` of the production dark-class matched-channel score is reproduced in the self-consistent harness; `~74 %` is production-only. Not ILLEGITIMATE (`ρ < 0.5`), not FLOOR-CONSISTENT (`ρ > 0.2`). Returns as fresh RULE (docket item R12) with the attribution question routed to `d-residual-attribution` behind the S0-B comparand (R4). Facts flagged: both legs negative (part of the residual is estimator-intrinsic at N=200 on a consistent universe); `delta_h_M` over-predicts the `−0.063` rail (linear-map non-linearity, cf. row #345 D4); the charter's `−0.14/event` is replaced by the first registered number `−0.197/dark event`. Revision counter: 0 consumed. Cost `≈ 0.2 CPU-h` local. Records: `exec/r-completion-residual/{READ_RECORD_rev4.md, completion_result_rev4_read.json, CHAIR_REDERIVATION_20260903.md, DESIGN_GATE_rev4_computability.md, BUILD_RECORD.md, completion_residual_reads.py}`. End-verifier re-derivation requested (appends to `END_VERIFICATION_20260903.md`).

**(4) MORNING DOCKET written:** `graph1_20260901/MORNING_DOCKET_20260904.md` — 11 findings F1–F11, 13 [RULE]s R1–R13 with the chair's flagged provisional readings, the veto list D1–D5, the backlog. Artifact to follow.

## Row #348 — 2026-09-03 ~23:50 — **SESSION CLOSE / handoff. All content chair-derived.**

**(1) End-verifier appendix for m-completion-residual landed** (`exec/END_VERIFICATION_20260903.md`): every number re-derived from RAW inputs matches (`T_prod −0.19663662 ± 0.01943993`, `Z −10.1151`; `T_harn −0.05054134 ± 0.00732173`, `Z −6.9029`; `ρ 0.25703`; `δh_M −0.091323`; `S_all` residual `0.0`; resolved flags `67/67`; disposition INTERMEDIATE (b) is the sole firing row). `s_M` confirmed the same quantity in both venues (identical header; `β̄(h)` identical at all three stencil nodes). **Two non-gating notes accepted as errata** in `exec/r-completion-residual/CHAIR_REDERIVATION_20260903.md`: **D10** the g-precision disclosure "rel `5.5e-3`" is a filename bug (h formatted to 2 dp); correct comparison rel `5.5e-8`/`3.5e-8`; **D11** row (b) is strict `0.2 < ρ < 0.5` (chair wrote `≤`; immaterial at `0.257`).

**(2) Morning docket ARTIFACT republished** at the same URL https://claude.ai/code/artifact/3cd5efee-f4d8-4cbf-977e-788698662305 (label "Morning docket 2026-09-04"): findings F1–F11, board, rulings R1–R13 with provisional readings, veto list D1–D5, backlog.
...
## Row #350 — 2026-09-04 ~00:45 — **b-dark-class-relative BUILD; migration HELD. All content chair-derived.**

**(1) R8 build done** (`exec/b-dark-class-relative/{dark_class.py, derive_tables.py, BUILD_RECORD.md}`; helper `is_dark_relative` at threshold `1e-6`, ruff/mypy clean; physics untouched).

**(2) Findings.** Max `L_cat/combined` ratio among the 157 moved events `9.75e-7` (continuous distribution, no natural gap — `1e-6` is a margin call); relative label stabilizes the S0-B-vs-08-27 split (`Δ = 4`, from 157); **BUT class counts under the relative label:** 08-27 `727/861` (exact `606/982`), S0-B truth `723/865` (`449/1139`), 09-02 re-baseline `iiib` `1241/347` (`606/982`), `joint_r1` `967/621` (`493/1095`) — **635 post-flip "hosted" events carry a catalogue-leg weight `< 1e-6` of the combined likelihood.**

**(3) Chair decision (flagged):** the 24-site `== 0` migration is **HELD**; the class definition returns as **[RULE] R14** (exact-zero support vs materiality threshold vs continuous covariate) — every A12 score-by-class read incl. row #335's catalogue-hosted localization is conditioned on the exact-zero label. `r-offset-subset` (batch 2) instructed to carry all three as frozen covariates so R14 gets a measurement. Chair note: `exec/b-dark-class-relative/CHAIR_NOTE_20260904.md`.

## Row #351 — 2026-09-04 ~01:05 — **r-offset-subset — the Graph 2 seed. All content chair-derived.**

**(1) Registration DRAFT written** by top-tier author A (`exec/r-offset-subset/REGISTRATION_DRAFT.md` 262 lines + `INFORMATION_FORECAST.md`): statistic `S` = top-k of the directional leave-one-out influence ranking under the frozen T0 convention (`k` banked from row #342: 82 `iiib`-2D primary; 94/72/46 replicates); 11 frozen covariates incl. the three class labels demanded by the R8 finding (exact-zero, relative `1e-6`, continuous `log10 f_cat`), `z`, log cone area, candidate retention/count, P6 outside, `log10 M` with the timeout-bin edge, SNR reported-only; per-covariate AUC (continuous) / Fisher OR (binary), Holm over `m = 11`; materiality = leave-out re-marginalisation of the enriched stratum vs a 1000-draw same-size null (seed `20260904`), `T_mat 0.008`; 2-of-3 replicate rule; dispositions SUBSET-IDENTIFIED / DIFFUSE-IN-COVARIATES (kill criterion) / INTERMEDIATE / INSTRUMENT, all fresh RULE, mandatory R14 line; three-agent blindness (blind covariate table → influence vector → join, sha256 gate); anchors row #302 `mean_h 0.6658540600`, k values, top-10 influence lists; cost zero cluster.

**(2) CHAIR DECISION (flagged, under the batch-2 renewed standing):** `d-offset-register` **RATIFIED-BY-CHAIR** conditional on a GREEN computability gate + byte-id + formula verifiers; workflow launched (gate → 3 disjoint sonnet builders → 2 verifiers → 1 disjoint reader; 7 sonnet agents).

## Row #352 — 2026-09-04 ~01:20 — **PA-HIER-34 S0-C addendum + d-sealed-register dossier + m1 launch block. All content chair-derived.**

**(1) Top-tier author B wrote (i) S0-C.** `exec/r-s0c-hgrid/REGISTRATION_ADDENDUM_PA-HIER-34.md` + `cluster/graph1_s0c_hgrid.sbatch` — the 5 registered θ-nodes at `h ∈ {0.665, 0.73, 0.78}`, config verbatim job `6779532` + `--h-nodes`; `h = 0.73` REUSED from `6779532` (driver diff `081b1f28..HEAD` empty, enforced as a run-time physics-freeze STOP) → 10 fresh cells (array 0–9, per-h out-roots); statistic `∂score/∂h` by a 3-point non-uniform Lagrange stencil at `0.73` (`Δ₋ 0.065`, `Δ₊ 0.050`), `Δh_θ = −S̄/D̄` with delta-method + bootstrap 3σ band; dispositions RESOLVED / NOT-RESOLVED / INSTRUMENT-DEFECT then IMMATERIAL-IN-h / MATERIAL-IN-h / INDETERMINATE vs `T_mat 0.008`, all CONDITIONAL-ON-R4; `g-znorm` proxy verified on banked S0-B (five `selection_tables` md5-identical). Cost `10 × 7.5 min × 16 cores ≈ 20 CPU-h` allocation-basis / `≈ 4 CPU-h` row #332 basis.
...
## Row #355 — 2026-09-04 ~02:15 — **R4 comparand read + timeout deep read. All content chair-derived.**

**(1) R4 comparand read** (`exec/d-photoz-leverage/R4_COMPARAND_READ.md`, sonnet + CHAIR CORRECTION): three-way diff at `h = 0.73`, `1588/1588` matched. **Pair B** (production CLI at `theta_sites 2.2`, job `6790708`) vs `c0prime_off_iiib` (`theta_sites all`): `combined_no_bh max_abs = 0` EXACTLY (`n = 0`), `L_cat_no_bh 0`, `B_num`/`D̃φ`/`αφ` `0` → `theta_sites` has **NO effect** on the production no-BH likelihood at `θ = (0,1)`. **Pair A** (comparand vs S0-B truth) `= Pair C` (S0-B truth vs `c0prime_off`): `combined_no_bh` `896` events, `max_abs 2.105e-3`, `max_rel 0.423`; `L_cat_no_bh` `1083` events, `max_rel 1.0` (zero-vs-nonzero); denominator columns identical. **The reader's summary sentence ("theta_sites explains the no_bh discrepancy") is REVERSED by its own numbers — corrected by the chair.** The S0-B DRIVER's `iiib` venue differs from the production CLI in the **CATALOGUE LEG**; the only flag it pins differently is `catalogue_numerator_survival_2d = off` (production default `mz_sel`/`eff`, the adopted `[P3-2D]` value `d4765539`; driver comment: keeps the banked Stage-0 comparands byte-identical). **R4b discriminating job launched:** production CLI + `theta_sites 2.2` + `mass_aware off` + `catalogue_numerator_survival_2d off`; if byte-identical to the S0-B truth node, the S0-B θ-pull was measured on a **PRE-ADOPTION counterfactual** of the production 2D setting (provenance fact for `d-photoz-leverage` R3–R5).

**(2) `rd-timeout-bin-seed61000`** (production pool, `exec/rd-timeout-bin-seed61000/`, sonnet; chair re-derived): SNR-stage timeouts `820/2412 = 0.340`; **M axis rates** `1.00 / 0.971 / 0.144 / 0.210 / 1.00` (Garwood max gradient `14.4σ`; binomial-SE `62.8σ`) → **NEW-SYSTEMATIC-CANDIDATE**; **p0 axis** `0.232 / 0.095 / 0.083 / 0.288 / 1.00` — `p0 > 20` is `100 %` timeout (`N = 483`), `13.6σ` → **NEW-SYSTEMATIC-CANDIDATE** (invisible in seed3000 because its retired `[10,16]` p0 prior could not sample the tail — design-gate deviation disclosed: p0 edges re-derived by the same blind rule, M edges reused); `e0` flat (`1.0σ`). `80.4 %` of the kept H0 population sits in the one M bin with `14 %` loss; the two extreme M bins lose `100 %`. `z`-depth `74 %` of ceiling. Chair facts: the inference population is the drawn population truncated to `M ∈ [~2e5, ~3.3e6] M☉` and `p0 ≲ 20`; whether this biases H0 hinges on `p_det` being built from the truncated pool — two Graph 2 candidates named (`q-timeout-selection-pdet`, `q-timeout-population-mismatch`). G7 row 8 → two-axis systematic candidate (docket item R9 sharpened).

## Row #356 — 2026-09-04 ~02:40 — **r-offset-subset gate + builds + verification. All content chair-derived.**

**(1) Computability gate GREEN** (`exec/r-offset-subset/DESIGN_GATE_computability.md`; one AMBER: the kill-criterion "verbatim" tag has no Graph-2 charter source yet — this node IS the seed; every disposition still returns as fresh RULE).

**(2) Three disjoint sonnet builders.** **B1** blind covariate table (`covariate_table_{iiib,joint_r1}.csv`, `1588` rows each, sha256 `90c92026…`/`fc2eebe7…`; `G-3a` `606 = 606` and `493 = 493` set-equality gates PASSED; `C10b n = 5` → NOT-TESTED disclosed; the R8 class counts reproduced exactly), **B2** influence vector (frozen T0 convention re-implemented verbatim with citation; md5 pins matched), **B3** scorer + synthetic check.

**(3) Byte-id verifier GREEN 30/30** (`mean_h 0.6658540599535224` vs anchor `0.6658540600`; `k = 82/94/72/46` exact; top-10 events exact; `k = 1588` endpoint `0.73`).

**(4) Formula-match verifier RED on the scorer:** (A) binary-covariate materiality stratum used the wrong enrichment direction; (B) the NOT-TESTED → INTERMEDIATE row had no code branch; (C) NaN handling in AUC/decile stratum undefined; (D) g-censoring rail gate not wired to NO-READ. Read correctly SKIPPED by the script. Fix round 2 → fresh formula re-gate → disjoint read launched.

## Row #357 — 2026-09-03 ~23:05 CEST (chair clock error in rows #349–#356: subtract ~3 h) — **Sealed m1 submitted; SSH ControlMaster lost to a concurrent-agent `ssh -O exit`. All content chair-derived.**

**(0) Wall-clock note.** The session's ledger timestamps since row #349 are CHAIR-ESTIMATED; the keepalive log shows the real time was `~23:00 CEST` on 2026-09-03 when the socket died. This row's date is written as `2026-09-03 ~23:05 CEST`.

**(1) Sealed-mock stage m1 SUBMITTED:** job `6790859`, array `0–40` (41 tasks), all PENDING at submit. Pin-at-first-touch (`exec/r-sealed-mock/PIN_RECORD.md`, dossier §5 PIN CORRECTION): the 0.67 run's injections resolve to `injection_pool_mix200k_20260728` (`707` files = the pool's own count); CRB md5 `8e9253fef42f574c569a04a3e19299ab`, `1345` data rows (`1343` + 2 empty per the local `combined_posterior_2d.json`; that run's `diagnostic_report.md` says `0` empty — flagged, unresolved). Only the pool-expectation lines of `cluster/graph1_sealed_m1_headstack.sbatch` changed (`500 → 707`, `depth15_50k → mix200k`). Record: `exec/batch2_cluster_ops/SUBMIT_RECORD_m1.md`.

**(2) INCIDENT (chair-owned).** Three agents polled `sacct` concurrently through the shared ControlMaster; the master began refusing new mux sessions; the R4b ops agent treated it as stale and ran `ssh -O exit bwunicluster`, destroying the only authenticated socket (OTP re-auth is author-only). **Consequence:** no polls/retrievals possible until the author re-authenticates; jobs `6790794` (S0-C), `6790859` (m1), `6790465` (GATE-ACC) keep running on the cluster. R4b sbatch prepared (`cluster/graph1_r4b_comparand_sites22_2doff.sbatch`: job-`6790708` CLI + `--catalogue_numerator_survival_2d off` `--catalogue_numerator_survival_2d_center` unset, the driver's pinned values) but **NOT submitted**. Lesson filed to memory (`ssh-controlmaster-agents-must-not-close`): serialize cluster access through one ops agent; never `ssh -O exit`.

## Row #358 — 2026-09-03 ~23:40 CEST (real clock; rows #349–#356 carry chair-estimated times ~3 h ahead, disclosed in row #357) — **r-timeout-selection registered; ERRATUM to row #355. All content chair-derived.**

**(1) `r-timeout-selection` REGISTRATION DRAFT** by top-tier author C (`exec/r-timeout-selection/{REGISTRATION_DRAFT.md 248 lines, INFORMATION_FORECAST.md, MECHANISM_NOTE.md, POOL_MANIFEST.md5}`). Mechanism, code-quoted: timed-out draws are ABSENT from pool and simulate results (`main.py:1293–1302`, `:763–771`) → every `p_det` is conditional on "completed in 90 s" (both budgets 90 s; G7 row 8's 30 s/90 s premise stale). **Q1** `q-timeout-selection-pdet`: sharing ratio `ρ(M bin)` and `Δmean_h` from `p_det × P_complete(M,z)` inside the mass quadratures (`g-closure` to `89,456` iterations), `T_mat 0.008`, three-valued. **Q2** `q-timeout-population-mismatch`: per-event information by M bin (reported-only), influence-vs-M, composition counterfactual re-weighted T0 posterior vs same-size null, width band `[0.80, 1.25]`. **NOT-covered [DO]s:** pool build-log fetch (`0 GPU-h`; Q1 capped INTERMEDIATE without it), node→GPU map, a `600-s` rescue re-run of the 822 logged parameter sets (`5–140 GPU-h`; kept waveforms generate in `0.16–0.62 s`, so `90 s` is a `>150×` regime), a longer timeout = runtime constant but implies a new pool (`~1000 + 50 GPU-h`), G7 row 8 text stale.

**(2) ERRATUM to row #355** (chair-verified): **(i)** the p0 axis is **WITHDRAWN** as a timeout systematic — kept `p0 ∈ [10.0025, 15.987]`; the live `[10,16]` bound (`parameter_space.py:95–106`) enforced by the stencil bounds check (`parameter_estimation.py:271–276`) drops `4,071/5,921` SNR-passers silently, so no non-timeout `p0 > 16` draw can be kept; this is the ratified D1 bounded null (row #94), not new. **(ii)** "`34 %`" is `P(timeout | kept ∪ timeout)`; per draw `822/89,456 = 0.92 %`. The M-axis shape stands at `~1 %` weight. Docket item R9 downgraded to a one-axis (M) selection whose H0 consequence is now registered. Erratum appended to `exec/rd-timeout-bin-seed61000/READ_RECORD.md`.

**(3) Chair decision.** `r-timeout-selection`'s gate/build/read waits for the `r-offset-subset` read (Q2 consumes its influence vector); Q1's pool build-log fetch waits for SSH re-auth.

## Row #359 — 2026-09-04 ~00:15 CEST (real clock) — **SSH ControlMaster loss: root cause + enforced fix. All content chair-derived.**

**(1) Author words, verbatim:** "okey this doesn't work via remote control. no ssh tonight unfortunately. can you please track down why it happened and ensure it does not happen again?"

**(2) ROOT CAUSE (two steps).** **(i)** the sealed-m1 submitter ran `timeout 350 ssh … 'sleep 300 …'` (a 5-min sleep INSIDE a remote session) while the R4b agent, the S0-C submitter's polls and the keepalive also held sessions on the shared ControlMaster → sshd's per-connection cap (`MaxSessions = 10`) refused new mux sessions ("mux_client_request_session: session request failed: Session open refused by peer", 4 occurrences in the R4b transcript); **(ii)** the R4b agent interpreted the refusal as a dead master and issued the `ssh -O exit` control command, destroying the only OTP-authenticated socket (keepalive log: OK at `23:02:39`, FAIL from `23:06:41`). Without step (ii) the refusals would have cleared when the sleeps ended.

**(3) FIX, ENFORCED (not advisory).** `.claude/hooks/ssh-guard.py` registered as a `PreToolUse` hook on Bash in `.claude/settings.json` — blocks closing the master (`ssh -O exit`/`-O stop`), deleting `~/.ssh/cm-*`, killing ssh processes, `sleep ≥ 60` inside a remote ssh command, and parallel ssh fan-out from one command (tested: 8 cases, exit 2 on each forbidden pattern, exit 0 on allowed ones; the hook already blocked two of the chair's own heredocs that merely quoted the patterns — working as intended). `cluster/agent_ssh.sh run|poll` — the only sanctioned agent path: 3-slot flock semaphore, local sleeps, backoff on refused mux, exit 3 with an author-only message if the socket is absent (verified tonight: exit 3). Cluster skill `SKILL.md` "ControlMaster discipline" section + `CLAUDE.md` "Cluster access discipline" paragraph (byte budget `23 217`). Memory: `ssh-controlmaster-agents-must-not-close` (feedback).

**(4) Consequence tonight.** No cluster access until the author logs in; jobs `6790794` (S0-C), `6790859` (sealed m1), `6790465` (GATE-ACC) run unattended; R4b not submitted; retrievals deferred to the morning (runbook 43 §1 to be amended).

## Row #360 — 2026-09-04 ~00:45 CEST — **r-offset-subset scorer rounds 2–3. All content chair-derived.**

**(1) Round 2** fixed the four rev-1 findings (binary enrichment direction, NOT-TESTED → INTERMEDIATE branch, NaN exclusion, censoring rail → NO-READ); fresh reviewer (`DESIGN_GATE_formula_rev2.md`) **RED on four more** (2D-vs-1D disagreement trigger absent, g-population join check absent, WEAK keyed on raw p not Holm, reported-only secondaries absent).

**(2) Round 3** fixed all five and produced a 40-item checklist; fresh enumerating reviewer (`DESIGN_GATE_formula_rev3.md`) confirms `40/40` formula items implemented and hand-verified, sha256 refusal live, CLI = parser — but **RED on two integration findings:** **(i) SCHEMA MISMATCH:** the scorer read bare ids (`C1…`) while the built tables carry suffixed ids (`C1_in_catalog`, …) and the influence CSVs lack the expected `_in_S`/`_d_e` columns; a total load failure would have silently banked DIFFUSE-IN-COVARIATES (no INSTRUMENT-DEFECT path) — a real defect, caught before any read; **(ii)** an alleged blindness leak: `BUILD_RECORD_B2.md` lists per-family top-10 influence tables.

**(3) CHAIR DECISIONS (flagged):** **(a)** draft PIN CORRECTIONS appended: the built files' headers are the schema of record; §8 phase-A/B script names corrected to `build_covariate_table.py`/`build_influence_vector.py`; missing registered column = hard INSTRUMENT-DEFECT; **(b)** finding (ii) is **NOT a breach** — the influence ranking is public by registration (its top-10 lists are the §G-2 byte-id anchors banked in row #342); the blind object is the covariate table (hashed) and the JOIN, which no agent has performed; disclosed, read proceeds.

**(4) Round 4 launched:** schema-aligned scorer + hard pre-flight + mandatory real-input `--dry-run` (loads/hash/schema/join counts, no aggregate) → fresh integration re-gate → disjoint read.

## Row #361 — 2026-09-04 ~01:20 CEST — **r-offset-subset round 4. All content chair-derived.**

**(1) Scorer aligned** to the built schemas (explicit `COVARIATE_COLUMN_MAP`; per-family `in_S` derived from the top-k rank; new `InstrumentDefectError` hard pre-flight: a missing registered column writes disposition INSTRUMENT-DEFECT and exits 1 — verified live); real-input `--dry-run` exit 0 per venue (`1588/1588` joined, `k 82/94` `iiib`, `72/46` `joint_r1`; sha256 matched); fresh integration reviewer GREEN (`DESIGN_GATE_formula_rev4.md`) with one carried open item: the influence CSVs carry no 41-node `ln L`, so the §4.2 materiality leave-out could not run.
...
## Row #368 — 2026-09-04 ~09:40 CEST — **Ratification WRITEBACKS. All content chair-derived.**

**(1) Append-only amendments** recording the R1–R18 rulings in the documents of record: `RESEARCH_GRAPH_1_PROPOSAL_20260901.md` "AMENDMENT 2 (R3)" — the `g-score-null` panel row now reads "runs on CONTROL venues; on production the truth-node score IS the registered read (PA-HIER-31(h)); a red there is a measurement" + §1.0 claim-layer writebacks (`c-rphi-mismatch` RETIRED; `c-a4-structural` VERIFIED; `q-a4-provisional` SETTLED; `q-cone-loss` SETTLED-refuted; `m-completion-residual` INTERMEDIATE (b); `q-postflip-calibration` NOT killed, S5 not launched, revision 2 PARKED; `c-theta-pull` CONJECTURED with a LIVE read attached; addendum A-0 RATIFIED; `r-offset-subset` primary SUBSET-IDENTIFIED); `GRAPH1_ADDENDUM_PROPOSAL_20260903.md` "RATIFICATION (2026-09-04)".

**(2) `docs/gates/G7_systematics_budget.md` row 8 UPDATE:** timeouts absent from `p_det`; `0.92 %`/draw; M-axis shape; p0 = D1 bound; grade unchanged.

**(3) R11 erratum** appended to the row #273 record `tree2_20260830/T1_3_ZWINDOW_P1_READOUT_RECORD.md` (a first copy landed on `exec/rd-parity-decomp/READ_RECORD.md` by the docs agent — kept with a cross-reference note).

**(4) `exec/b-dark-class-relative/CHAIR_NOTE_20260904.md`** "RULING R14".

## Row #369 — 2026-09-04 ~10:05 CEST — **r-highz-completion registered (Graph 2 mechanism node). All content chair-derived.**

**(1) REGISTRATION DRAFT** (Graph 2 mechanism node opened by R15; top-tier author D; `exec/r-highz-completion/{REGISTRATION_DRAFT.md 266 lines, INFORMATION_FORECAST.md, MECHANISM_NOTE.md}`). **Three corrections to the chair's brief, from banked tables:** **(1)** the zero-candidate class is `606` events in `iiib` (C7 == 0 ≡ C2 exact-zero dark), not `159` — the `159` was a tie-broken bottom decile; registered target `K_dark` = top-z decile ∩ zero-candidate = `144` (of `K = 159`), reference `R` = low-z half of the remaining dark class = `231`; `joint_r1` `493/111/48/191`; all sha256-pinned. **(2)** The code identity for a zero-candidate event is TWO-term: `ln L^2D = ln B_num + ln(B_num_wbh/B_num) − ln D̃_φ(h)` (`bayesian_statistics.py:6741–6746` with `:2497`), with `D̃_φ` event-independent → the completion denominator cancels identically in any z-differential (registered as an identity gate, `Δ_D ≡ 0`); `p_det` is not separable (inside `B_num` via the fused `S̄_φ`, `:6408`, and inside `g`, `:6519`); `L_comp` is diagnostic-only. **(3)** The harness mapping follows R12's convention (harness reproduces the pull ⇒ estimator-internal), the brief's inversion flagged as fresh RULE **R-b**.
...
## Row #376 — 2026-09-04 ~12:45 CEST — **r-timeout-selection Q1 computability gate RED at registration level. All content chair-derived.**

**(1) Gate result** (fresh sonnet, 11 checks; `exec/r-timeout-selection/DESIGN_GATE_Q1_computability.md`) **RED at the registration level, no build:** **(1)** §4 cites the per-task aggregate timeout line (`main.py:1349–1352`, no M) as the source of a per-M-bin statistic; the per-draw `TimeoutError` line that carries M (`~main.py:1293–1302`; `5,040` occurrences in the fetched build logs) is not registered; **(2)** the fetched build logs hold `1,070` task attempts vs the pool's `707` CSVs — `707` completion lines in the current "separatrix-sign" format, `326` older-format, `37` crashed (matches the pool's `code_rev` split `f6449051`/`a9f29e82`); no rule says which attempts feed S1.1; aggregate vs per-draw timeout counts differ by `45`; **(3)** the S1.3 denominator leg names `precompute_completion_denominator` (`:1170`), which has NO mass quadrature (mass-integrated rate cancels by its own comment); the mass-integrated `p_det` lives in the private `_mass_trunc_denominator_inner_m_integral` (`:869`); **(4)** the numerator insertion (`completion_mass_factor_g_sel`'s `s_query` Callable, `:2276`) is unnamed and the 10 `p_det` call sites are not classified. **AMBER:** g-closure(i) arithmetic (SNR-stage-only closure `423`, not `384`; `3,488` includes `39` CRB-stage `ZeroDivisionError`s), the new build-log manifest unpinned (self-referential entry). **GREEN:** core pins, `[A13]` gate, node list, blindness/p0 fence.

**(2) Chair decision.** Q1 revision 1 requested from author C (5 items; thresholds untouched; pre-launch design revisions declared not to consume the post-disposition counter).

**(3) Also recorded** (author word, backlog): the clerk found **NO pre-commit hook installed** in this checkout (`.git/hooks/pre-commit` absent) — the ruff/mypy gates ran manually for the row #375 code commit; `uv run pre-commit install` is the fix (CLAUDE.md assumes it).

## Row #377 — 2026-09-04 ~13:05 CEST — **r-timeout-selection Q1 revision 1. All content chair-derived.**

**(1) Revision 1** by author C (`REGISTRATION_DRAFT.md` now `380` lines — the append-only revisions exceed the `320`-line cap, flagged, not trimmed): **(1)** S1.1 source = the per-draw `TimeoutError` line (`main.py:1293–1302`; `injection_campaign :1143` at the build commit; `5,040` raw → `2,520` deduped); the aggregate line is the g-closure tally only. **(2)** Population rule: exactly the `707` attempts whose CSV is in `POOL_MANIFEST.md5` (seed/task-id match); `363` excluded and disclosed; the `2,475`-vs-`2,520` gap attributed to the `37` crashed attempts, reported-only. **(3)** Denominator leg: `D'(h)` **DROPPED** from the primary (`precompute_completion_denominator` has no mass axis, `:1284`/`:1292–1297`); re-registered on `_mass_trunc_denominator_inner_m_integral` (`:869`, call site `denominator_integrant_with_bh_mass :8048–8058`) via a duck-typed `detection_probability` proxy; thinned-pool composition leg secondary. **(4)** Numerator: the `s_query` Callable (`:2276`) named as the zero-edit insertion; the 10 `p_det` call sites tabulated (`P_complete` on every with-BH query, none on without-BH). **(5)** g-closure(i) corrected to `3,449`/residual `423`; build-log MANIFEST pinned `6ae9c109…` (self-referential row disclosed); the revision counter declared unspent by design rounds.

**(2) Q1 chain rerun launched** (gate must return `≥ 8` real checks).

## Row #378 — 2026-09-04 ~13:30 CEST — **r-highz-completion round 1. All content chair-derived.**

**(1) Computability gate GREEN** (populations `606/144/231` `iiib` and `493/111/191` `joint_r1` re-derived and sha256-matched; harness pooled anchors `12,060`/`4,826`/`1,207`/`1,148` exact; 5-row closure residuals `~1e-15`; line numbers re-pinned `HEAD↔1ec9514d`, one hunk of `+17`; two AMBERs: the harness-manifest sha256 byte format under-specified, the non-additivity band without a CLI flag).

**(2) Build** (`b-highz-decomp → highz_decomp_reads.py`): real-input `--dry-run` exit 0 with every pin and count matched; the builder caught and fixed a bug where the checkpoint's `n_scored` (always `200`) was used instead of the CSV's scored count (`13,400` vs the `12,060` anchor); reverse-engineered the manifest format (sorted `"{seed} {md5}"` of `event_likelihoods.csv` only, no trailing newline) to byte-exactness; `--nonadditivity-max` flag added; synthetic fixture exercises every disposition row.

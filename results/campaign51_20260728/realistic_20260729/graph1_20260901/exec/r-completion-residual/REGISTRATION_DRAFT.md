...
| **INTERMEDIATE (d) HARNESS-ONLY-SIGNAL** | \|Z_harn\| > 3 AND \|Z_prod\| ≤ 3 (ρ undefined) | c-residual-floor-consistent SUPPORTED on production at this N (bound: \|T_prod\| + 3·SE_prod); c-residual-illegitimate SUPPORTED on the harness universe only (T_harn ± SE_harn) — the production dark class is null while the estimator's own-model universe shows a signal; the production residual is bounded, the harness signal is unattributed | fresh RULE to the author: interpretation (an inconsistency the production realisation does not resolve at N = 1512, or a harness-population effect); optional cell R (§6) within the cap as a replication of the harness signal |
...
| 1b (computability re-gate) | §4: the reachable, unregistered combination \|Z_harn\| > 3 AND \|Z_prod\| ≤ 3 (ρ undefined) now has its own row, INTERMEDIATE (d) HARNESS-ONLY-SIGNAL, returning as a fresh RULE; no threshold touched. Confirmation: the registered Z_harn is the BETWEEN-UNIVERSE matched-channel statistic — S_M,harn,U is computed per harness universe U from the per-universe diagnostics file `tree2_20260830/b8_cal_harness_work_s4_postflip/seed9010NN_S/simulations/diagnostics/event_likelihoods.csv` (dark labels from the sibling `simulations/prepared_cramer_rao_bounds.csv`, `host_galaxy_index = −1`), then SE_harn = SD_U/√67; the checkpoint's `score_at_truth.no_bh.dark.mean` (full score) is used ONLY for the byte-id gate, never for Z_harn. |

...
| R15 | m-offset-subset: INTERMEDIATE by the literal (vacuous 2D/1D) trigger; primary family SUBSET-IDENTIFIED — the 82 offset-carrying events are high-z (AUC 0.87, median z 0.85), 82 % exact-zero DARK (zero catalogue candidates), low-SNR; top-z decile leave-out +0.086; removing the whole dark class overshoots truth (0.822) | Ratify SUBSET-IDENTIFIED for the primary with the 1D data-contract gap disclosed; open the Graph 2 mechanism node "completion leg at high z with zero-candidate cones" (ties to R12's 74 % production-only residual) | #362, #363 |
...
| R18 | Launch r-timeout-selection Q2 (population-mismatch counterfactual; zero compute): registration at revision 2 + chair erratum (820 SNR-stage timeouts as the pinned population); gate rev2 RED only on that erratum item | Approve; rerun gate→build→read with the same disjoint pattern; Q1 after the pool build-log fetch | #364–#366 |
...
## 8. Morning batch additions (rows #367–#383)
| # | ruling | chair's provisional reading | inputs |
|---|---|---|---|
| R19 | m-timeout-q2: POPULATION-MISMATCH-MATERIAL (Δmean_h −0.011, 3.6σ; influence M-structured ρ 0.23) — the 90-s timeout truncation biases the estimate UP by ~0.011 (masks part of the offset) | Ratify; the remedy is the injection-pipeline [DO] (longer timeout / rescue re-run, 5–140 GPU-h) — author's word | #374 |
| R20 | Q1 (p_det conditioning) PARKED after 3 design rounds: the pool is a heterogeneous stack (old-format subpopulation in-pool, a crashed task's file in the pool, colliding task ids) and the registered denominator site is never dispatched by the production anchor (gaussian kernel branch) | Rule the pool population of record and the denominator branch of record; or accept Q2 as the answer and close Q1 bounded-undetermined | #383 |
| R21 | S0-B provenance: the driver's H_BOUNDS (0.50, 0.86) widened the candidate z-window vs production (h_min 0.60); truth-node test at (0.60, 0.86) submitted (6794615, queued); if byte-identical to R4b, the full 5-node S0-B is re-run at the production window (≈2 CPU-h) and the θ-pull read is repeated there | Approve the production-window S0-B re-run on byte-identity; the row #336 read stays REPORTED-ONLY on the wide-window venue | #372, #375, #382 |
| R22 | S0-C: b-axis h-derivative resolved (Z 7.4) but Δh_θ ≈ 0.25 saturates; s-axis h-flat → the θ-pull is not mappable to an h displacement at this order; d-residual-attribution item 3 enters as "LIVE in θ, unmappable in h" | Ratify (conditional on R21) | #371 |
| R23 | r-highz-completion (R15 mechanism node): registered, built, in gate round 3; read pending | Ratify the register node retroactively; the read returns as its own RULE | #369, #378, #381 |
| R24 | m-highz-completion (R15 mechanism node) READ: TERM-OWNS(B) in all four families — the B_num completion-numerator term owns 93 % of the high-z zero-candidate pull; freezing the 144 events' term profiles removes the entire 2D offset (+0.064); the harness reproduces it (ρ_S 1.04, Z −81) → ESTIMATOR-INTERNAL candidate by the R12 convention | RULE the mapping: illegitimate (estimator-internal) vs floor-consistent (correct likelihood's h-slope for information-poor high-z dark events); then authorize b-highz-bnum-factor (split p_det vs S̄_φ inside B_num) | #385 |
Backlog additions: pre-commit hook was not installed in this checkout (now installed, row #376); GATE-ACC 6790465 has sat PENDING (Priority) since 2026-09-03.

## 9. Midday batch additions (2026-09-04, rows #387–; sealed m1 read)
Chair state: sealed-mock stage (m1) job 6794421 COMPLETED 41/41 (57.6 core-h, cap 75); retrieved, all read-time gates GREEN except g-censoring (RED → BOUND by the dossier's own rule); read record `exec/r-sealed-mock/READ_RECORD_m1.md`, top-tier verifier appendix in §6 of that file. **No cell call made by the chair.**
| # | ruling | chair's provisional reading (flagged; "Ratified" accepts it) | inputs |
|---|---|---|---|
| R25 | **(m1) cell disposition.** 2D: MAP 0.630, mean 0.6277, σ 0.0140; 1D: 0.630 / 0.6311 / 0.0139 (1343 events, truth 0.67 public). Against the registered table BOTH cells fire literally: TUNED (\|mean − 0.6659\| = 2.73σ ≤ 3σ, \|MAP − 0.665\| = 2.50σ ≤ 3σ; 1D 2.58σ/2.51σ agree) AND NOT-TUNED-AT-RAIL (edge-node mass at 0.600 = 0.044 ≫ 1e-3, 71 % of mass at h ≤ 0.63). The table's "neither" INTERMEDIATE cell does not cover a both-fire posterior | The chair reads NEITHER as banked. Options for the author: (a) rule the rail clause dominant → NOT-TUNED-AT-RAIL, read as a BOUND (mean ≤ 0.63 at 0.600-floor, TRANSFER unresolved), G7d not refuted-in-form but the 0.67 pool does NOT reproduce the 0.6659 tuned value; (b) rule the 3σ clause dominant → TUNED at 2.5–2.7σ, HALT paper claims per draft §7 (the chair flags that a posterior 0.038 below the anchor with < 1 % mass above 0.66 is not what "tuned to 0.666" was written to mean); (c) INTERMEDIATE by fiat, banked, and the band question returns with the (m2) sealed run. The chair recommends (a) with the σ-distance reported alongside, and an erratum to the registration text closing the both-fire gap before (m2) | #387, READ_RECORD_m1.md |
| R26 | **joint_r1 sibling of (m1).** Dossier §1 item 6 condition (iiib ≤ 60 core-h) is satisfied at 57.6 core-h; the sbatch is ready (`VENUE=joint_r1`, ≈ 75 core-h cap, ≈ 6 wall-h at the fairshare floor) | The batch-2 grant has lapsed; NOT submitted. [DO] if the author wants the second venue before (m2); the chair's view: only useful if R25 resolves to (a) or (c), since a venue replicate cannot resolve the registration-text ambiguity | #387 |
| R27 | **Registration erratum for (m2).** The cell table needs a precedence rule (rail clause before the 3σ clause, or an explicit "both fire → NOT-TUNED-AT-RAIL" row) and a stated treatment of a posterior narrower than the anchor's σ (m1 σ_h = 0.014 vs re-baseline 0.0185 at larger N) | [DO] the chair drafts the erratum as a dossier section for author ratification; no (m2) launch until ratified | READ_RECORD_m1.md §4 |
Pending from §8 unchanged: R19–R24. Cluster: 6794615 (S0-B truth node, 0.60 window) and 6790465 (GATE-ACC) still PENDING at the fairshare floor — nothing to read.
| R28 | **(m1) pool pin override.** The registration (dossier §2/§5) named `injection_pool_depth15_50k` (500 files) as the p_det pool and STOP-gated on it; the 0.67 run actually links `injection_pool_mix200k_20260728` (707 files). The chair re-pinned to the actual pool before submission (PIN CORRECTION) — a post-registration correction the author has not seen. Same-pool fact (chair-verified this session): the 0.73 re-baseline run `run_20260729_seed61000` links the identical pool (manifest list-md5 `75f4030d5d…` on both), so the anchor comparison is same-pool | Ratify the correction as a registration erratum (the intent "the pool the 0.67 simulate itself used" is met); or rule the (m1) read NO-READ under the registered text | READ_RECORD_m1.md §2, PIN_RECORD.md, D_SEALED_REGISTER_DOSSIER.md "PIN CORRECTION" |

"""AMENDMENT-2/3 registered V-flat cells (seed 20271118, shared across pair)."""
import json, time
from pathlib import Path
from darksiren_emri.validation.pp_coverage import PPCoverageConfig, run_coverage, D50_GPC
mult = json.loads(Path("pretuning/CHOSEN_VFLAT.json").read_text())["d50_mult"]
for sc in ("fused", "off"):
    cid = f"vflat_250_production_{sc}"
    ...
    res = run_coverage(PPCoverageConfig(
        ...
        seed=20271118, kernel="volume", catalogue_mode=True, mixture_mode="absolute",
        z_support=0.40, sky_frac=1e-4, n_galaxies=200_000, mass_channel=True,
        mass_horizon_index=0.25, selection_cell=sc, gw_measurement_scatter=False,
        h_step=0.004, d50_gpc=D50_GPC * mult))
    out.write_text(json.dumps(res))
    print(f"{cid}: done in {time.perf_counter()-t0:.0f}s", flush=True)
print("VFLAT COMPLETE")

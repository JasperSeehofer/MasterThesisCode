...
* :func:`draw_dark_host` / :func:`draw_dark_hosts` -- draws of out-of-catalog
  ...
  :class:`~darksiren_emri.galaxy_catalogue.handler.HostGalaxy` objects with
...
density is :func:`darksiren_emri.emri_rate.R_EMRI`. With the default
...
class _CompletenessModel(Protocol):
    ...
    :class:`~darksiren_emri.galaxy_catalogue.glade_completeness.GladeCatalogCompleteness`
...
class _RateWeightedHostSource(Protocol):
    ...
    :class:`~darksiren_emri.galaxy_catalogue.handler.GalaxyCatalogueHandler`
...
def draw_dark_hosts(
    ...
    * **Omega-independent completeness (limiting case a)** -- otherwise ``z`` is the
    ...
    :class:`~darksiren_emri.galaxy_catalogue.handler.HostGalaxy` with

...
_Generated by `analyze_failures.py` on 2026-03-31. Data source: Scenario A (injection CSVs available locally)._

### Scenario A: Injection CSV Analysis

Found 262 CSV files across 7 h-values.

#### Summary by h-value

```
   h  tasks  events events/task  detections det_rate_% z_median z_max M_median
 0.6     38   22500         592          29       0.13    0.467 1.500   187300
0.65     40   26000         650          40       0.15    0.455 1.500   196314
 0.7     40   23500         588          35       0.15    0.467 1.500   195413
0.73     40   25500         638          60       0.24    0.458 1.500   185540
 0.8     40   25000         625          40       0.16    0.461 1.500   192178
0.85     40   25500         638          92       0.36    0.451 1.500   200246
 0.9     24   17000         708          67       0.39    0.397 1.500   194556
```

**Total events across all h-values:** 165,000
**Total detections (SNR >= 20):** 363
**Overall detection rate:** 0.220%

#### Key Observations

1. **Detection rate is very low (0.22%):** Only 363 out of 165,000 successful SNR computations produce SNR >= 20. This means the vast majority of GPU time is spent computing SNR for undetectable events.

2. **This data predates the z_cut = 0.5 optimization:** z_max = 1.500 in the CSV data, while the current code (after Phase 11.1 optimization) applies z_cut = 0.5. Events at z > 0.5 represent 33.7% of the data (55,663 events) but produce **zero** detections. The z_cut optimization saves ~34% of compute.

3. **Task completion variation:** 195 tasks completed with 500 events, 66 tasks with 1000, and 1 with 1500. This reflects the `simulation_steps` parameter varying across SLURM array submissions (likely different campaign batches).

4. **Detection yield correlates with h:** Higher h-values (0.85, 0.9) produce more detections per event. This is expected: higher H0 means smaller d_L for the same z, hence higher SNR.

#### Failure Rate Limitation

**Cannot compute waveform failure rate from CSV data.** The CSV records only successful SNR computations. The `injection_campaign()` while loop runs until it accumulates `simulation_steps` successes, so each CSV has exactly that many rows regardless of how many attempts failed. The failure rate is only available from SLURM log progress messages (`{counter} / {iteration}`), which were not rsynced to the local machine.

**No SLURM logs were found locally** in `cluster/logs/`. To obtain failure rate data:
```bash
rsync -avP bwunicluster:/path/to/workspace/logs/ cluster/logs/
```

#### Detections by Redshift Bin

```
z bin                 total   detected   det_rate
--------------------------------------------------
(0.00, 0.05]            370        181     48.92%
(0.05, 0.10]           1280        155     12.11%
(0.10, 0.15]           2767         26      0.94%
(0.15, 0.20]           4905          1      0.02%
(0.20, 0.30]          18056          0      0.00%
(0.30, 0.50]          81959          0      0.00%
(0.50, 1.00]          19856          0      0.00%
(1.00, 1.50]          35807          0      0.00%
```

**Detection horizon:** All detections are at z < 0.2, with 93% at z < 0.1. The z_cut = 0.5 in the current code is generous; a tighter z_cut = 0.2 would save an additional ~60% of compute within the z < 0.5 range (but would be risky if the population model or h-value shifts the horizon).

#### Detections by log10(M) Bin

```
log10(M) bin          total   detected   det_rate
--------------------------------------------------
(4.0, 4.5]                0          0      0.00%
(4.5, 5.0]            43699          5      0.01%
(5.0, 5.5]            70597         98      0.14%
(5.5, 6.0]            50704        260      0.51%
(6.0, 6.5]                0          0      0.00%
(6.5, 7.0]                0          0      0.00%
```

**Detection mass range:** Detections concentrate at M ~ 10^5 - 10^6, with 72% in the (10^5.5, 10^6] bin. No detections at M < 10^4.5 or M > 10^6. The M distribution from `Model1CrossCheck` appears to sample only the 10^4.5 - 10^6 range (due to `_apply_model_assumptions()` constraints), not the full ParameterSpace range of 10^4 - 10^7.

#### Detection Distribution

Detected events (SNR >= 20):
- z: median=0.050, range=[0.008, 0.155]
- M: median=427,445, range=[93,221, 994,841]
- SNR: median=29.6, range=[20.1, 276.7]

### Data Gap: Waveform Failure Breakdown by Exception Type

**Cannot be assessed from current local data.** The injection CSV does not record failed events (see CSV Design Limitation section above). SLURM logs are not available locally. The data gap blocks quantitative failure-by-type and failure-by-region analysis.

**Concrete proposal for Phase 18 or next campaign:**
1. Rsync SLURM logs from bwUniCluster for the current campaign
2. Implement failure CSV recording (see Future Tracking Proposal above)
3. Run `analyze_failures.py --log-dir <path>` once logs are available

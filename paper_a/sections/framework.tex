...
$\pm 4\sigma_{d_L}$ distance image, where $\sigma_{d_L}$ is the
event's $1\sigma$ GW luminosity-distance uncertainty from its Fisher
analysis, and the host's $\pm 4\sigma_z$ window, respectively), with
$p_\mathrm{det}$ the detection probability
...
$\sigma_z$ for a generic per-galaxy scatter when the galaxy label is
irrelevant.  The kernel takes the general form
\begin{equation}
  p_g(z) \;=\;
...
$\mathrm{d}V_\mathrm{c}/\mathrm{d}z$ reads
\begin{equation}
  w_\mathrm{pop}(z) \;\equiv\;
  \frac{1}{1+z}\,
  \frac{\mathrm{d}V_\mathrm{c}}{\mathrm{d}z}.
  \label{eq:wpop}
\end{equation}
% src: docs/derivations/G2b_host_z_volume_prior.md Eq. (1.3); docs/derivations/G2a_completion_sky_marginal_4pi.md Eq. (4)
For an isotropic population the per-steradian host density is
$w_\mathrm{pop}(z)/(4\pi)$, and it is in this per-steradian form that
the measure enters every sky-resolved integral below.
...
($p_\mathrm{det}$ vanishes beyond the injection horizon).  The
constant $1/(4\pi)$ converting the full-sky measure \eqref{eq:wpop} to
its per-steradian form is kept explicit here and in
equation~\eqref{eq:bnum}; it is common to $B_\mathrm{num}$, $D$, and
$\beta_{\bar G}$ and cancels in the assembled likelihood below.
$D(h)$ is
...
\emph{explicitly}, as the measure $w_\mathrm{pop}(z)\,\mathrm{d}z$ (in
its per-steradian form $w_\mathrm{pop}/4\pi$) of

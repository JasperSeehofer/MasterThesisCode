...
three-dimensional localisation region (galaxies outside it are
...
\begin{equation}
  p_g(z) \;=\;
  \frac{\mathcal{N}\big(z_g;\, z,\, \sigma_{z,g}\big)\;\Pi(z)}{Z_g},
  \qquad
  Z_g = \int \mathrm{d}z'\;
  \mathcal{N}\big(z_g;\, z',\, \sigma_{z,g}\big)\;\Pi(z'),
  \label{eq:kernel}
\end{equation}
% src: docs/derivations/G2b_host_z_volume_prior.md Eq. (1.4)
where $\mathcal{N}(z_g; z, \sigma_{z,g})$ is the likelihood of the
...
decomposition \citep{MISSING:Bishop2006-PRML},
% MISSING CITATION: C. M. Bishop, "Pattern Recognition and Machine Learning", Springer 2006, Eqs. (2.81)-(2.82)
...
$\big(\Pi,\, \mathcal{G}_i^{\rm sel}\big)$: with the bare kernel

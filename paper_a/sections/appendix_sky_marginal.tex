...
numerator is
\begin{equation}
  B_{\rm num}(h) = \int_{z_-}^{z_+} \mathrm{d}z\, \big[1 - f_k(z)\big]
  \left[ \int_{S^2} \frac{\mathrm{d}\Omega}{4\pi}\,
  \mathcal{N}_3\big((\phi,\theta,u(z,h));\,\mu,\Sigma\big) \right]
  \frac{\mathrm{d}V_c}{\mathrm{d}z\,\mathrm{d}\Omega}\, \frac{1}{1+z},
  \label{eq:app:sky:bnum}
\end{equation}
where $f_k(z)$ is the catalogue completeness in the event's sky pixel,
$u(z,h) = d_L(z,h)/\hat d_L$, and the $1/(1+z)$ converts to the detector-frame
rate \citep{Mandel:2018mve}.
...
$\bar p_{\rm GW}(u) = (4\pi)^{-1}\,\mathcal{N}_1(u;1,\Sigma_{uu})\,
...
$\Delta\Omega \approx 0.2\,{\rm deg}^2 = 6.1\times10^{-5}\,{\rm sr}$
($\sigma_\theta \approx \sigma_\phi \approx 3.3\times10^{-3}\,{\rm rad}$), % src: docs/derivations/G2a_completion_sky_marginal_4pi.md §4
...
under the beam; dropping it over-weights $B_{\rm num}$ relative to the
in-catalogue term $\beta_G L_{\rm cat}$ by $1/\sin\hat\theta$ per event ---
...
${\rm rad}^2 \leftrightarrow {\rm sr}$ identification under which
$B_{\rm num}$ and $\beta_G L_{\rm cat}$ are dimensionally commensurable is
...
\citep{MISSING:Gray2022-pixelated}. % MISSING CITATION: Gray et al. 2022, "A pixelated approach to galaxy catalogue incompleteness: improving the dark siren measurement of the Hubble constant", MNRAS 512, 1127, arXiv:2111.04629
...
$f_k \to 0$, $B_{\rm num}$ reduces to the catalogue-free volume-prior
...
2/(\sigma_\phi\sigma_\theta)$, is $\approx 1.6\times10^{3}$ at a $2^\circ$ sky
error and $\approx 1.8\times10^{5}$ at the median $0.2\,{\rm deg}^2$ % src: docs/derivations/G2a_completion_sky_marginal_4pi.md §7 item 5

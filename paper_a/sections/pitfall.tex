...
\begin{equation}
  \delta z_{\mathrm{Edd}} = \sigma_z^{2}\, q(z_g),
  \qquad
...
\begin{equation}
  \Delta h \simeq -\,h \left.\frac{\mathrm{d}\ln d_L}{\mathrm{d}z}\right|_{\bar{z}} \sigma_z^{2}\, q(\bar{z})
...
where $\bar{z}$ is a representative detected-host redshift; the derivation, the population-averaging of the Eddington bias coefficient $K(\bar z)$, and the exact-quadrature cure are given in Appendix~\ref{sec:app:voldeconv}.
...
and the implied coefficient $-\Delta h/\sigma_z^2 \approx 17$--$20$ matches $K(\bar z)$ at $\bar z \approx 0.25$--$0.27$, inside the synthetic detected population. % src: docs/derivations/G2b_host_z_volume_prior.md section 2.3
...
For a GLADE-like catalogue the effect is not even perturbative. At a host redshift $z_g = 0.05$ the expansion parameter of Eq.~\eqref{eq:pitfall:eddz} is $\sigma_z\, q(z_g) = 1.33$ already at $\sigma_z = 0.035$, % src: docs/derivations/G2b_host_z_volume_prior.md section 2.3

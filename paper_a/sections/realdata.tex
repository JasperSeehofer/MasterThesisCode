...
The matrix above rests on the 494-event subsample and its coarse seven-point grid. A full-scale confirmation applies the production pipeline with the volume-deconvolved default to the full seed600 sample over the complete 38-value grid spanning $[0.60, 0.86]$; after the pipeline's internal quality cuts, $3343$ events enter the combination. The de-rail persists at production resolution: the posterior peaks in the grid interior at $\mathrm{MAP} = 0.745$ (mean $0.744$) with edge mass below $10^{-75}$, and the MBH-mass channel concurs ($\mathrm{MAP} = 0.745$, mean $0.745$, edge mass below $10^{-66}$). % src: results/commission_20260701/redteam/combined_posterior_voldeconv_fullgrid.json + combined_posterior_voldeconv_fullgrid_with_bh_mass.json (cluster eval array 5732036 + task-32 retry 5735964, combine 5735965; commit 6d4c4e1, CLI-default volume_deconv, no explicit mode flag)
At full-sample precision the 68 per cent credible set narrows to $[0.740, 0.745]$, so the $+0.015$ displacement from the injected value is statistically resolved; it is consistent in sign, and roughly in magnitude, with the $+0.010$ offset that the volume-deconvolved estimator already showed on the subsample (mean $0.740$). % src: combined_posterior_voldeconv_fullgrid.json (mass-sorted 68 per cent set 0.740--0.745; mean 0.7441)

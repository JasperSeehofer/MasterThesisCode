"""Adversarial verification of the photo-z volume-REGULARISED numerator kernel.

Replicates run_closure_photoz with an added `regularised_kernel` flag that swaps the
bare Gaussian numerator kernel for k_g = N(z;z_g,sigma_z) * p_bg(z) / Z_g, keeping the
GLOBAL Option-A selection denominator (gd) UNCHANGED. Compares against standard and the
disqualified consistent_denom control.
"""
    ...
    for i, h in enumerate(hs):
        ...
        for d_meas, sdL in events:
            ...
            if regularised_kernel:
                # k_g = N * p_bg / Z_g, Z_g = INT N p_bg dz. h-independent p_bg shape.

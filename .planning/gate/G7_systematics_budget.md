...
| 5 | Global-denominator Option-A cancellation (Σ_global vs h³β_G) | −17% end-to-end h-shape tilt → 'global' mode rails | **AVOIDED** (local modes; 'global' deprecated + warned) | `.planning/gate/G1_beta_g_check.md`; ablation cube `G3_ablation_cube.json` |
...
| 8 | Waveform-timeout selection (30 s inj / 90 s sim asymmetry) | 0.6–1.25% of events per stage removed; does not cancel in p_det; bounded sub-% on H₀ | **CAMPAIGN**: params now logged at all catch sites `4d1c27a` → bin by (M, e₀, p₀); expect higher rates at the deeper post-dt² population | `.planning/gate/G9_timeout_scan.md` |

...
def self_fixture() -> ParameterEstimation:
    return ParameterEstimation(
        waveform_generation_type="FastSchwarzschildEccentricFlux"
    )
...
def test_finite_differences_constant(
    self, waveform: np.ndarray[float], neighouring_waveform: np.ndarray[float]
) -> None:
